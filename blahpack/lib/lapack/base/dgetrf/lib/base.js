...
*/
function dgetrf( M, N, A, strideA1, strideA2, offsetA, IPIV, strideIPIV, offsetIPIV ) { // eslint-disable-line max-len, max-params
	var iinfo;
	var info;
	var minMN;
	var sa1;
	...
	for ( j = 0; j < minMN; j += NB ) {
		...
		dlaswp( j, A, sa1, sa2, offsetA, j, j + jb - 1, IPIV, strideIPIV, offsetIPIV, 1 );

		if ( j + jb < N ) {
			// Apply interchanges to columns j+jb..N-1
			dlaswp( N - j - jb, A, sa1, sa2, offsetA + ( j + jb ) * sa2,
				j, j + jb - 1, IPIV, strideIPIV, offsetIPIV, 1
			);
