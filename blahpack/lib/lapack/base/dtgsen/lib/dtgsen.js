...
*/
function dtgsen( order, ijob, wantq, wantz, SELECT, strideSELECT, N, A, LDA, B, LDB, ALPHAR, strideALPHAR, ALPHAI, strideALPHAI, BETA, strideBETA, Q, LDQ, Z, LDZ, M, pl, pr, DIF, strideDIF, WORK, strideWORK, lwork, IWORK, strideIWORK, offsetIWORK, liwork ) { // eslint-disable-line max-len, max-params
	var sa1;
	var sa2;
	var sb1;
	var sb2;
	...
	if ( order === 'column-major' && LDQ < max( 1, M ) ) {
		throw new RangeError( format( 'invalid argument. Nineteenth argument must be greater than or equal to max(1,M). Value: `%d`.', LDQ ) );
	}
	...
	if ( order === 'column-major' && LDZ < max( 1, M ) ) {
		throw new RangeError( format( 'invalid argument. 21th argument must be greater than or equal to max(1,M). Value: `%d`.', LDZ ) );
	}
	...
	return base( ijob, wantq, wantz, SELECT, strideSELECT, 0, N, A, sa1, sa2, 0, B, sb1, sb2, 0, ALPHAR, strideALPHAR, 0, ALPHAI, strideALPHAI, 0, BETA, strideBETA, 0, Q, sq1, sq2, 0, Z, sz1, sz2, 0, M, pl, pr, DIF, strideDIF, 0, WORK, strideWORK, 0, lwork, IWORK, strideIWORK, offsetIWORK, liwork ); // eslint-disable-line max-len
}
