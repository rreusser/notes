...
* @param {Complex128Array} d - output diagonal sign array
* @param {integer} strideD - stride length for `d`
* @throws {TypeError} first argument must be a valid order
* @throws {RangeError} second argument must be a nonnegative integer
* @throws {RangeError} third argument must be a nonnegative integer
* @throws {RangeError} fifth argument must be greater than or equal to max(1,M) (or max(1,N) for row-major)
* @returns {integer} status code (0 = success)
*/
function zlaunhr_col_getrfnp2( order, M, N, A, LDA, d, strideD ) { // eslint-disable-line max-len, max-params
	var sa1;
	var sa2;
	...
	return base( M, N, A, sa1, sa2, 0, d, strideD, 0 ); // eslint-disable-line max-len
}
