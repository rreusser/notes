...
var zsytrs3 = require( './../../zsytrs_3/lib/base.js' );


// MAIN //

/**
* Estimates the reciprocal of the condition number (in the 1-norm) of a complex symmetric matrix `A` using the factorization `A = P*U*D*U^T*P^T` or `A = P*L*D*L^T*P^T` computed by `zsytrf_rk`.
*
...
* @param {Complex128Array} A - factored matrix from `zsytrf_rk` (column-major)
...
*/
function zsycon3( uplo, N, A, strideA1, strideA2, offsetA, e, strideE, offsetE, IPIV, strideIPIV, offsetIPIV, anorm, rcond, WORK, strideWORK, offsetWORK ) {
	var ainvnm;
	var upper;
	var ISAVE;
	var KASE;
	...
	// Check that the diagonal matrix `D` is nonsingular. For complex symmetric, the diagonal can be fully complex; both real and imaginary parts must be zero.
	if ( upper ) {
		for ( i = N - 1; i >= 0; i -= 1 ) {
	...
	while ( true ) {
		...
		// Multiply by `inv(A)`: solve `A*X = WORK[0..N-1]` using `zsytrs_3`.
		zsytrs3( uplo, N, 1, A, strideA1, strideA2, offsetA, e, strideE, offsetE, IPIV, strideIPIV, offsetIPIV, WORK, sw, N * sw, offsetWORK );
	}
