...
var dsytri3 = require( './../lib/ndarray.js' );


// FIXTURES //

var fixtureDir = path.join( __dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' ); // eslint-disable-line max-len
var lines = readFileSync( path.join( fixtureDir, 'dsytri_3.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
function runFixture( uplo, N, tc ) {
	...
	info = dsytri3( uplo, N, A, 1, N, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0, lwork ); // eslint-disable-line max-len
...
test( 'dsytri_3: N=1 lower trivial inverse', function t() {
	...
	lwork = ( 1 + NB + 1 ) * ( NB + 3 );
	work = new Float64Array( lwork );
	info = dsytri3( 'lower', 1, A, 1, 1, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0, lwork ); // eslint-disable-line max-len
	assert.equal( info, tc.info, 'info' );
...
test( 'dsytri_3: N=0 quick return', function t() {
	...
	info = dsytri3( 'lower', 0, A, 1, 1, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0, 10 ); // eslint-disable-line max-len
	assert.equal( info, 0, 'info' );
});

test( 'dsytri_3: workspace query returns optimal LWORK', function t() {
	var ipiv;
	var work;
	var info;
	var tc;
	...
	info = dsytri3( 'lower', 5, A, 1, 5, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0, -1 ); // eslint-disable-line max-len
	assert.equal( info, tc.info, 'info' );
	assertClose( work[ 0 ], tc.work1, 1e-14, 'work1' );
});
...
test( 'dsytri_3: validation throws on invalid uplo', function t() {
	...
	assert.throws( function fn() {
		dsytri3( 'invalid', 2, A, 1, 2, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0, 40 );
	}, TypeError );
});
