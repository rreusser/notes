...
function zgbtrf( M, N, kl, ku, AB, strideAB1, strideAB2, offsetAB, IPIV, strideIPIV, offsetIPIV ) {
	...
	// Get Float64 view of AB for direct element access
	...
	WORK13v = reinterpret( WORK13, 0 );
	WORK31v = reinterpret( WORK31, 0 );

	// Initialize WORK13 upper triangle to zero (already zero from allocation)
	// Initialize WORK31 lower triangle to zero (already zero from allocation)
	...
	for ( j = 0; j < minMN; j += NB ) {
		...
		for ( jj = j; jj < j + jb; jj++ ) {
			...
			if ( ABv[ idx ] !== 0.0 || ABv[ idx + 1 ] !== 0.0 ) {
				...
				// Scale multipliers: compute 1/AB(kv, jj) then zscal
				idx = ( offsetAB + kv * sa1 + jj * sa2 ) * 2;
				TEMP[ 0 ] = 1.0;
				TEMP[ 1 ] = 0.0;
				cmplx.divAt( TEMP, 0, TEMP, 0, ABv, idx );
				zscal( km, new Complex128( TEMP[ 0 ], TEMP[ 1 ] ),
		...
		if ( j + jb < N ) {
			...
			for ( i = 0; i < j3; i++ ) {
				...
				for ( ii = j + i; ii < j + jb; ii++ ) {
					...
					if ( ip !== ii ) {
						...
						idx2 = ( offsetAB + ( kv + ip - jj ) * sa1 + jj * sa2 ) * 2;
						tempR = ABv[ idx ];
						tempI = ABv[ idx + 1 ];
						ABv[ idx ] = ABv[ idx2 ];
						ABv[ idx + 1 ] = ABv[ idx2 + 1 ];
						ABv[ idx2 ] = tempR;
						ABv[ idx2 + 1 ] = tempI;
					}
				}
			...
			if ( j3 > 0 ) {
				...
				for ( jj = 0; jj < j3; jj++ ) {
					for ( ii = jj; ii < jb; ii++ ) {
						idx = ( ii + jj * LDWORK ) * 2;
						idx2 = ( offsetAB + ( ii - jj ) * sa1 + ( jj + j + kv ) * sa2 ) * 2;
						WORK13v[ idx ] = ABv[ idx2 ];
						WORK13v[ idx + 1 ] = ABv[ idx2 + 1 ];
					}
				}
				...
				for ( jj = 0; jj < j3; jj++ ) {
					for ( ii = jj; ii < jb; ii++ ) {
						idx = ( ii + jj * LDWORK ) * 2;
						idx2 = ( offsetAB + ( ii - jj ) * sa1 + ( jj + j + kv ) * sa2 ) * 2;
						ABv[ idx2 ] = WORK13v[ idx ];
						ABv[ idx2 + 1 ] = WORK13v[ idx + 1 ];
					}
				}
			}
		} else {
