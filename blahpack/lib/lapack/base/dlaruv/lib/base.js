...
// MM table: 128 x 4. Each row i contains the multiplier for the i-th power
// of the base multiplier modulo 2^48. Stored as flat array, row-major.
// MM[i][j] = MM_TABLE[ i*4 + j ], i = 0..127, j = 0..3
// prettier-ignore
var MM_TABLE = [ // eslint-disable-line max-lines
	494,322,2508,2549,
	2637,789,3754,1145,
	255,1440,1766,2253,
	2008,752,3572,305,
...
function dlaruv( iseed, strideISEED, offsetISEED, N, x, strideX, offsetX ) { // eslint-disable-line max-len, max-params
	...
	for ( i = 0; i < lv; i++ ) {
		...
		mi = i * 4; // row offset into MM_TABLE

		// Retry loop: if the result rounds to exactly 1.0, perturb seed and retry
		while ( true ) { // eslint-disable-line no-constant-condition
			...
			// Convert 48-bit integer to a real number in (0,1)
			v = R * ( it1 + R * ( it2 + R * ( it3 + R * it4 ) ) );

			if ( v !== 1.0 ) {
				break;
			}
			// If rounded to exactly 1.0, perturb seed and retry
			i1 = i1 + 2;
			i2 = i2 + 2;
			i3 = i3 + 2;
			i4 = i4 + 2;
