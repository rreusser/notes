...
* where `A` is an N-by-N symmetric positive definite matrix stored in packed format.
*
...
*/
function dppsv( uplo, N, nrhs, AP, B, LDB ) {
	...
	if ( LDB < max( 1, N ) ) {
		throw new RangeError( format( 'invalid argument. Sixth argument must be greater than or equal to max(1,N). Value: `%d`.', LDB ) );
	}
	return base( uplo, N, nrhs, AP, 1, 0, B, 1, LDB, 0 );
}
