...
var dlarzb = require( './../lib/base.js' );
...
test( 'dlarzb: unsupported direct returns early without modifying C', function t() {
	var WORK = new Float64Array( 4 );
	var V = new Float64Array( 4 );
	var T = new Float64Array( 4 );
	var C = new Float64Array( [ 1.0, 2.0, 3.0, 4.0 ] );
	...
	if ( C[0] !== 1 || C[1] !== 2 || C[2] !== 3 || C[3] !== 4 ) {
		throw new Error( 'C should be unchanged for unsupported direct' );
	}
});

test( 'dlarzb: unsupported storev returns early without modifying C', function t() {
	var WORK = new Float64Array( 4 );
	var V = new Float64Array( 4 );
	var T = new Float64Array( 4 );
	var C = new Float64Array( [ 1.0, 2.0, 3.0, 4.0 ] );
	...
	if ( C[0] !== 1 || C[1] !== 2 || C[2] !== 3 || C[3] !== 4 ) {
		throw new Error( 'C should be unchanged for unsupported storev' );
	}
});
