...
>
> The reference LAPACK routine calls `BLAS_ZGBMV_X` and `BLAS_ZGBMV2_X` from the
...
> double-precision `zgbmv` for both calls, so the `prec_type` parameter has **no
> precision effect** — residuals are computed in plain double precision.
> Additionally, a modified reference Fortran source is maintained at
> `pipeline/zla_gbrfsx_extended/base_only.f` (strips the XBLAS calls and forces
> `Y_PREC_STATE=BASE_RESIDUAL`) so fixtures can be generated. Do **not** rely on
> this module for extra-precise residuals until XBLAS is ported.
...
-   Extended-precision residual paths (`BLAS_ZGBMV_X`, `BLAS_ZGBMV2_X`) are not available in this build; the routine always uses the standard `zgbmv` residual. The `prec_type` parameter is accepted for API compatibility but has no effect.
...
// See `examples/index.js` for a complete diagonal 2x2 system example.
console.log( typeof zla_gbrfsx_extended );
```
