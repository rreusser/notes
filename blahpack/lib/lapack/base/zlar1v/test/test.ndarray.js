...
var zlar1v = require( './../lib/ndarray.js' );


// FIXTURES //

var fixtureDir = path.join( __dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
var lines = readFileSync( path.join( fixtureDir, 'zlar1v.jsonl' ), 'utf8' ).trim().split( '\n' );
...
/**
* Computes the Rayleigh residual for a real-valued vector stored in a reinterpreted complex view.
*
* @private
* @param {Array} td - diagonal
* @param {Array} te - sub-diagonal
* @param {number} lambda - shift
* @param {Float64Array} zr - real view of the complex eigenvector (re/im interleaved)
* @param {integer} b1 - 1-based start
* @param {integer} bn - 1-based end
* @returns {number} residual
*/
function tridiagResid( td, te, lambda, zr, b1, bn ) {
	var sum;
	var nz;
	var rr;
	var zi;
...
/**
* Invokes `zlar1v` with a fresh output buffer set and returns the outputs.
*
* @private
* @param {Array} factors - LDL factors
...
* @returns {Array} outputs `[Zv, ISUPPZ, negcnt, ztz, mingma, r, nrminv, resid, rqcorr]`
*/
function runCase( factors, n, b1, bn, lambda, gaptol, wantnc, rin ) {
	...
	zlar1v( n, b1, bn, lambda, factors[ 0 ], 1, 0, factors[ 1 ], 1, 0, factors[ 2 ], 1, 0, factors[ 3 ], 1, 0, pivmin, gaptol, Z, 1, 0, wantnc, negcnt, ztz, mingma, r, ISUPPZ, 1, 0, nrminv, resid, rqcorr, WORK, 1, 0 );

	return [ Zv, ISUPPZ, negcnt[ 0 ], ztz[ 0 ], mingma[ 0 ], r[ 0 ], nrminv[ 0 ], resid[ 0 ], rqcorr[ 0 ] ];
}


// TESTS //

test( 'zlar1v ndarray: tridiag5 smallest eigenvector (Rayleigh residual)', function t() {
	var factors;
	var lambda;
	var out;
	var td;
	...
	for ( i = 0; i < 5; i += 1 ) {
		assert.equal( out[ 0 ][ ( 2 * i ) + 1 ], 0.0, 'im(z[' + i + ']) = 0' );
	}

	s = 0.0;
	for ( i = 0; i < 5; i += 1 ) {
		s += out[ 0 ][ 2 * i ] * out[ 0 ][ 2 * i ];
	}
...
test( 'zlar1v ndarray: tridiag5 middle eigenvector (lambda=4)', function t() {
	var factors;
	var out;
	var td;
	var te;
...
test( 'zlar1v ndarray: tridiag3 largest eigenvector (fixture match up to sign)', function t() {
	var factors;
	var sign;
	var dot;
	var out;
	...
	dot = 0.0;
	for ( i = 0; i < 3; i += 1 ) {
		dot += out[ 0 ][ 2 * i ] * tc.z[ 2 * i ];
	}
	sign = ( dot >= 0 ) ? 1.0 : -1.0;
	for ( i = 0; i < 3; i += 1 ) {
		assertClose( sign * out[ 0 ][ 2 * i ], tc.z[ 2 * i ], 1e-12, 're(z[' + i + '])' );
		assert.equal( out[ 0 ][ ( 2 * i ) + 1 ], 0.0, 'im(z[' + i + ']) = 0' );
	}
...
test( 'zlar1v ndarray: subrange b1=2 bn=4, wantnc=false', function t() {
	var factors;
	var out;
	var td;
	var te;
	...
	assert.equal( out[ 0 ][ 0 ], 0.0, 're(z[0])=0' );
	assert.equal( out[ 0 ][ 1 ], 0.0, 'im(z[0])=0' );
	assert.equal( out[ 0 ][ 8 ], 0.0, 're(z[4])=0' );
	assert.equal( out[ 0 ][ 9 ], 0.0, 'im(z[4])=0' );
...
test( 'zlar1v ndarray: preset twist r=3', function t() {
	var factors;
	var out;
	var td;
	var te;
	...
	assert.equal( out[ 0 ][ 4 ], 1.0, 're(z[2]) = 1' );
	assert.equal( out[ 0 ][ 5 ], 0.0, 'im(z[2]) = 0' );

	rr = tridiagResid( td, te, 4.0, out[ 0 ], 1, 5 );
	assert.ok( rr < 1e-10, 'Rayleigh residual small: ' + rr );
});

test( 'zlar1v ndarray: huge gaptol truncates both sweeps', function t() {
	var factors;
	var out;
	var td;
	var te;
	...
	assert.equal( out[ 0 ][ 4 ], 1.0, 're(z[2])=1' );
	assert.equal( out[ 0 ][ 2 ], 0.0, 're(z[1])=0 (truncated back)' );
	assert.equal( out[ 0 ][ 6 ], 0.0, 're(z[3])=0 (truncated forward)' );
	assert.ok( out[ 1 ][ 0 ] > 1, 'isuppz[0] shrunk' );
	assert.ok( out[ 1 ][ 1 ] < 5, 'isuppz[1] shrunk' );
});

test( 'zlar1v ndarray: b1 != 1 initial WORK seed', function t() {
	var factors;
	var out;
	var td;
	var te;
	...
	assert.equal( out[ 0 ][ 0 ], 0.0, 're(z[0])=0' );
	assert.equal( out[ 0 ][ 2 ], 0.0, 're(z[1])=0' );
	assert.equal( out[ 1 ][ 0 ], 3, 'isuppz[0]=b1' );
	assert.equal( out[ 1 ][ 1 ], 6, 'isuppz[1]=bn' );
});

test( 'zlar1v ndarray: stride support', function t() {
	var factors;
	var mingma;
	var nrminv;
	var rqcorr;
	...
	zlar1v( 5, 1, 5, 4.0, D2, 2, 0, L2, 2, 0, LD2, 2, 0, LLD2, 2, 0, 1e-300, 0.0, Z2, 2, 0, true, negcnt, ztz, mingma, r, ISUPPZ, 1, 0, nrminv, resid, rqcorr, WORK, 1, 0 );

	zr = new Float64Array( 2 * 5 );
	for ( i = 0; i < 5; i += 1 ) {
		zr[ 2 * i ] = Zv[ 4 * i ];
		zr[ ( 2 * i ) + 1 ] = Zv[ ( 4 * i ) + 1 ];
	}
	rr = tridiagResid( td, te, 4.0, zr, 1, 5 );
	assert.ok( rr < 1e-10, 'strided z Rayleigh residual: ' + rr );
	assert.ok( ztz[ 0 ] > 0, 'ztz > 0' );
});

test( 'zlar1v ndarray: offset support', function t() {
	var factors;
	var mingma;
	var nrminv;
	var rqcorr;
	...
	zlar1v( 3, 1, 3, 4.0, D, 1, 2, L, 1, 2, LD, 1, 2, LLD, 1, 2, 1e-300, 0.0, Z, 1, 2, true, negcnt, ztz, mingma, r, ISUPPZ, 1, 0, nrminv, resid, rqcorr, WORK, 1, 0 );

	zr = new Float64Array( 2 * 3 );
	for ( i = 0; i < 3; i += 1 ) {
		zr[ 2 * i ] = Zv[ 2 * ( 2 + i ) ];
		zr[ ( 2 * i ) + 1 ] = Zv[ ( 2 * ( 2 + i ) ) + 1 ];
	}
	rr = tridiagResid( td, te, 4.0, zr, 1, 3 );
