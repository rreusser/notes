...
test( 'zlantp throws TypeError for invalid uplo', function t() {
	assert.throws( function throws() {
		zlantp( 2, 'invalid', 'non-unit', new Float64Array( 4 ), new Float64Array( 4 ), new Float64Array( 4 ) );
	}, TypeError );
});

test( 'zlantp throws TypeError for invalid diag', function t() {
	assert.throws( function throws() {
		zlantp( 2, 'upper', 'invalid', new Float64Array( 4 ), new Float64Array( 4 ), new Float64Array( 4 ) );
	}, TypeError );
});

test( 'zlantp throws RangeError for negative N', function t() {
	assert.throws( function throws() {
		zlantp( 2, 'upper', 'non-unit', -1, new Float64Array( 4 ), new Float64Array( 4 ) );
	}, RangeError );
});
