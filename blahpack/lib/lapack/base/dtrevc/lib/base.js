/* eslint-disable max-len, max-params, max-depth, no-var, max-statements, no-mixed-operators, no-unused-vars, max-lines-per-function, max-lines */
...
/**
* Computes eigenvectors of a real upper quasi-triangular matrix T (unblocked).
*
* @private
...
*/
function dtrevc( side, howmny, SELECT, strideSELECT, offsetSELECT, N, T, strideT1, strideT2, offsetT, VL, strideVL1, strideVL2, offsetVL, VR, strideVR1, strideVR2, offsetVR, mm, M, WORK, strideWORK, offsetWORK ) { // eslint-disable-line no-unused-vars
	var rightv;
	var bothv;
	var leftv;
	var somev;
	...
	// [N..2N-1]: real part of eigenvector (Fortran WORK(1+N) .. WORK(N+N))
	// [2N..3N-1]: imaginary part of eigenvector (Fortran WORK(1+2N) .. WORK(N+2N))
	n2 = 2 * N;
	...
	if ( rightv ) {
		...
		for ( ki = N - 1; ki >= 0; ki-- ) {
			if ( ip === -1 ) {
				// Previous iteration was second of complex pair; skip first
				ip = 1;
				continue;
			} else if ( ki === 0 ) {
				ip = 0;
			...
			if ( ip === 0 ) {
				...
				for ( k = 0; k < ki; k++ ) {
					WORK[ offsetWORK + N + k ] = -T[ oT + k * sT1 + ki * sT2 ];
				}
				...
				for ( j = ki - 1; j >= 0; j-- ) {
					...
					if ( j1 === j2 ) {
						...
						// 2x2 diagonal block
						res = dlaln2( false, 2, 1, smin, ONE, T, sT1, sT2, oT + ( j - 1 ) * sT1 + ( j - 1 ) * sT2, ONE, ONE, WORK, 1, N, offsetWORK + N + j - 1, wr, ZERO, X, 1, 2, 0 );
						scale = res.scale;
						xnorm = res.xnorm;
				...
				if ( over ) {
					...
					if ( ki > 0 ) {
						dgemv( 'no-transpose', N, ki, ONE, VR, strideVR1, strideVR2, offsetVR, WORK, 1, offsetWORK + N, WORK[ offsetWORK + N + ki ], VR, strideVR1, offsetVR + ki * strideVR2 );
					}
					...
					dcopy( ki + 1, WORK, 1, offsetWORK + N, VR, strideVR1, offsetVR + is * strideVR2 );
				...
				if ( Math.abs( T[ oT + ( ki - 1 ) * sT1 + ki * sT2 ] ) >= Math.abs( T[ oT + ki * sT1 + ( ki - 1 ) * sT2 ] ) ) {
					...
					WORK[ offsetWORK + n2 + ki ] = wi / T[ oT + ( ki - 1 ) * sT1 + ki * sT2 ];
				} else {
					WORK[ offsetWORK + N + ki - 1 ] = -wi / T[ oT + ki * sT1 + ( ki - 1 ) * sT2 ];
					WORK[ offsetWORK + n2 + ki ] = ONE;
				}
				WORK[ offsetWORK + N + ki ] = ZERO;
				WORK[ offsetWORK + n2 + ki - 1 ] = ZERO;

				// Form right-hand side
				for ( k = 0; k < ki - 1; k++ ) {
					WORK[ offsetWORK + N + k ] = -WORK[ offsetWORK + N + ki - 1 ] * T[ oT + k * sT1 + ( ki - 1 ) * sT2 ];
					WORK[ offsetWORK + n2 + k ] = -WORK[ offsetWORK + n2 + ki ] * T[ oT + k * sT1 + ki * sT2 ];
				}
				...
				for ( j = ki - 2; j >= 0; j-- ) {
					...
					if ( j1 === j2 ) {
						...
						if ( scale !== ONE ) {
							dscal( ki + 1, scale, WORK, 1, offsetWORK + N );
							dscal( ki + 1, scale, WORK, 1, offsetWORK + n2 );
						}
						WORK[ offsetWORK + N + j ] = X[ 0 ];
						WORK[ offsetWORK + n2 + j ] = X[ 2 ];

						// Update right-hand side
						daxpy( j, -X[ 0 ], T, sT1, oT + j * sT2, WORK, 1, offsetWORK + N );
						daxpy( j, -X[ 2 ], T, sT1, oT + j * sT2, WORK, 1, offsetWORK + n2 );
					} else {
						// 2x2 diagonal block
						...
						WORK[ offsetWORK + N + j - 1 ] = X[ 0 ];
						WORK[ offsetWORK + N + j ] = X[ 1 ];
						WORK[ offsetWORK + n2 + j - 1 ] = X[ 2 ];
						WORK[ offsetWORK + n2 + j ] = X[ 3 ];

						// Update right-hand side
						daxpy( j - 1, -X[ 0 ], T, sT1, oT + ( j - 1 ) * sT2, WORK, 1, offsetWORK + N );
						daxpy( j - 1, -X[ 1 ], T, sT1, oT + j * sT2, WORK, 1, offsetWORK + N );
						daxpy( j - 1, -X[ 2 ], T, sT1, oT + ( j - 1 ) * sT2, WORK, 1, offsetWORK + n2 );
						daxpy( j - 1, -X[ 3 ], T, sT1, oT + j * sT2, WORK, 1, offsetWORK + n2 );
					}
				}
				...
				if ( over ) {
					...
					if ( ki > 1 ) {
						...
						dgemv( 'no-transpose', N, ki - 1, ONE, VR, strideVR1, strideVR2, offsetVR, WORK, 1, offsetWORK + n2, WORK[ offsetWORK + n2 + ki ], VR, strideVR1, offsetVR + ki * strideVR2 );
					} else {
						dscal( N, WORK[ offsetWORK + N + ki - 1 ], VR, strideVR1, offsetVR + ( ki - 1 ) * strideVR2 );
						dscal( N, WORK[ offsetWORK + n2 + ki ], VR, strideVR1, offsetVR + ki * strideVR2 );
					}
					...
					dcopy( ki + 1, WORK, 1, offsetWORK + n2, VR, strideVR1, offsetVR + is * strideVR2 );
	...
	if ( leftv ) {
		...
		for ( ki = 0; ki < N; ki++ ) {
			if ( ip === 1 ) {
				// Previous was first of complex pair; skip second
				ip = -1;
				continue;
			} else if ( ki === N - 1 ) {
				ip = 0;
			...
			if ( ip === 0 ) {
				...
				if ( Math.abs( T[ oT + ki * sT1 + ( ki + 1 ) * sT2 ] ) >= Math.abs( T[ oT + ( ki + 1 ) * sT1 + ki * sT2 ] ) ) {
					...
					WORK[ offsetWORK + n2 + ki + 1 ] = ONE;
				} else {
					WORK[ offsetWORK + N + ki ] = ONE;
					WORK[ offsetWORK + n2 + ki + 1 ] = -wi / T[ oT + ( ki + 1 ) * sT1 + ki * sT2 ];
				}
				WORK[ offsetWORK + N + ki + 1 ] = ZERO;
				WORK[ offsetWORK + n2 + ki ] = ZERO;

				// Form right-hand side
				for ( k = ki + 2; k < N; k++ ) {
					WORK[ offsetWORK + N + k ] = -WORK[ offsetWORK + N + ki ] * T[ oT + ki * sT1 + k * sT2 ];
					WORK[ offsetWORK + n2 + k ] = -WORK[ offsetWORK + n2 + ki + 1 ] * T[ oT + ( ki + 1 ) * sT1 + k * sT2 ];
				}
				...
				for ( j = ki + 2; j < N; j++ ) {
					...
					if ( j1 === j2 ) {
						...
						if ( WORK[ offsetWORK + j ] > vcrit ) {
							rec = ONE / vmax;
							dscal( N - ki, rec, WORK, 1, offsetWORK + N + ki );
							dscal( N - ki, rec, WORK, 1, offsetWORK + n2 + ki );
							vmax = ONE;
							vcrit = BIGNUM;
						}

						WORK[ offsetWORK + N + j ] -= ddot( j - ki - 2, T, sT1, oT + ( ki + 2 ) * sT1 + j * sT2, WORK, 1, offsetWORK + N + ki + 2 );
						WORK[ offsetWORK + n2 + j ] -= ddot( j - ki - 2, T, sT1, oT + ( ki + 2 ) * sT1 + j * sT2, WORK, 1, offsetWORK + n2 + ki + 2 );
						...
						if ( scale !== ONE ) {
							dscal( N - ki, scale, WORK, 1, offsetWORK + N + ki );
							dscal( N - ki, scale, WORK, 1, offsetWORK + n2 + ki );
						}
						WORK[ offsetWORK + N + j ] = X[ 0 ];
						WORK[ offsetWORK + n2 + j ] = X[ 2 ];
						vmax = Math.max( Math.abs( WORK[ offsetWORK + N + j ] ), Math.abs( WORK[ offsetWORK + n2 + j ] ), vmax );
						vcrit = BIGNUM / vmax;
					} else {
						// 2x2 diagonal block
						beta = Math.max( WORK[ offsetWORK + j ], WORK[ offsetWORK + j + 1 ] );
						...
						WORK[ offsetWORK + n2 + j + 1 ] -= ddot( j - ki - 2, T, sT1, oT + ( ki + 2 ) * sT1 + ( j + 1 ) * sT2, WORK, 1, offsetWORK + n2 + ki + 2 );
				...
				if ( over ) {
					...
					if ( ki < N - 2 ) {
						...
						dgemv( 'no-transpose', N, N - ki - 2, ONE, VL, strideVL1, strideVL2, offsetVL + ( ki + 2 ) * strideVL2, WORK, 1, offsetWORK + n2 + ki + 2, WORK[ offsetWORK + n2 + ki + 1 ], VL, strideVL1, offsetVL + ( ki + 1 ) * strideVL2 );
					} else {
						dscal( N, WORK[ offsetWORK + N + ki ], VL, strideVL1, offsetVL + ki * strideVL2 );
						dscal( N, WORK[ offsetWORK + n2 + ki + 1 ], VL, strideVL1, offsetVL + ( ki + 1 ) * strideVL2 );
					}
					...
					dcopy( N - ki, WORK, 1, offsetWORK + N + ki, VL, strideVL1, offsetVL + ki * strideVL1 + is * strideVL2 );
					dcopy( N - ki, WORK, 1, offsetWORK + n2 + ki, VL, strideVL1, offsetVL + ki * strideVL1 + ( is + 1 ) * strideVL2 );
