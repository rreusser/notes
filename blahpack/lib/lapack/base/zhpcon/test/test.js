...
var zhpcon = require( '../lib/base.js' );
...
/**
* Runs a fixture-based test: loads factored AP and IPIV, runs zhpcon, and verifies rcond against fixture.
*
* @private
...
function runFixtureTest( name, uplo, N ) {
	...
	info = zhpcon( uplo, N, AP, 1, 0, IPIV, 1, 0, tc.anorm, rcond, WORK, 1, 0 );
...
test( 'zhpcon is a function', function t() {
	assert.strictEqual( typeof zhpcon, 'function' );
});

test( 'zhpcon: N=0 returns rcond=1', function t() {
	var rcond;
	var WORK;
	var IPIV;
	var info;
	...
	rcond = new Float64Array( 1 );
	info = zhpcon( 'upper', 0, AP, 1, 0, IPIV, 1, 0, 0.0, rcond, WORK, 1, 0 );
	assert.strictEqual( info, tc.info );
	assert.strictEqual( rcond[ 0 ], tc.rcond );
});

test( 'zhpcon: upper, 3x3, well-conditioned', function t() {
	runFixtureTest( 'upper_well_cond', 'upper', 3 );
});

test( 'zhpcon: lower, 3x3, well-conditioned', function t() {
	runFixtureTest( 'lower_well_cond', 'lower', 3 );
});

test( 'zhpcon: N=1 (upper)', function t() {
	runFixtureTest( 'n_one_upper', 'upper', 1 );
});

test( 'zhpcon: N=1 (lower)', function t() {
	runFixtureTest( 'n_one_lower', 'lower', 1 );
});

test( 'zhpcon: identity 3x3 (upper)', function t() {
	runFixtureTest( 'identity_upper', 'upper', 3 );
});

test( 'zhpcon: identity 3x3 (lower)', function t() {
	runFixtureTest( 'identity_lower', 'lower', 3 );
});

test( 'zhpcon: ill-conditioned diagonal (upper)', function t() {
	runFixtureTest( 'ill_cond_upper', 'upper', 3 );
});

test( 'zhpcon: singular 3x3 (upper, rcond=0)', function t() {
	runFixtureTest( 'singular_upper', 'upper', 3 );
});

test( 'zhpcon: 4x4 (upper)', function t() {
	runFixtureTest( '4x4_upper', 'upper', 4 );
});

test( 'zhpcon: 4x4 (lower)', function t() {
	runFixtureTest( '4x4_lower', 'lower', 4 );
});

test( 'zhpcon: real diagonal (upper)', function t() {
	runFixtureTest( 'real_diag_upper', 'upper', 3 );
});

test( 'zhpcon: anorm=0 returns rcond=0', function t() {
	var rcond;
	var WORK;
	var IPIV;
	var info;
	...
	rcond = new Float64Array( 1 );
	info = zhpcon( 'upper', 2, AP, 1, 0, IPIV, 1, 0, 0.0, rcond, WORK, 1, 0 );
