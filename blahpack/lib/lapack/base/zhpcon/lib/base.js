...
*
* The routine uses the factorization A = U_D_U^H or A = L_D_L^H computed by
* zhptrf. An estimate is obtained for norm(inv(A)), and the reciprocal of the
* condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).
*
* IPIV uses the same 0-based convention as zhptrf:
*
...
*/
function zhpcon( uplo, N, AP, strideAP, offsetAP, IPIV, strideIPIV, offsetIPIV, anorm, rcond, WORK, strideWORK, offsetWORK ) {
	var ainvnm;
	var upper;
	var ISAVE;
	var KASE;
	...
	while ( true ) {
		...
		zhptrs( uplo, N, 1, AP, sap, offsetAP, IPIV, strideIPIV, offsetIPIV, WORK, sw, N * sw, ow );
	}
