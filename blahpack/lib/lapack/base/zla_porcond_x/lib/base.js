...
/**
* Estimates the infinity norm condition number of `op(A)*diag(X)` for a Hermitian positive-definite matrix.
*
...
* have been factored by `zpotrf` into `AF`.
*
* ## Notes
*
...
* @param {Complex128Array} AF - Cholesky factor from `zpotrf`
...
*/
function zla_porcond_x( uplo, N, A, strideA1, strideA2, offsetA, AF, strideAF1, strideAF2, offsetAF, x, strideX, offsetX, WORK, strideWORK, offsetWORK, RWORK, strideRWORK, offsetRWORK ) {
	var ainvnm;
	var anorm;
	var ISAVE;
	var prodR;
	...
	while ( true ) { // eslint-disable-line no-constant-condition
		...
		if ( KASE[ 0 ] === 2 ) {
			...
			// Solve using the Cholesky factorization.
			if ( upper ) {
