...
test( 'dtpttf: N=5, no-transpose, lower with stride=2 and offset', function t() {
	...
	assert.equal( info, 0, 'info should be 0' );
	for ( i = 0; i < nt; i++ ) {
		assert.equal( actual[ 2 * i ], expected[ i ], 'ARF[' + i + '] mismatch with stride=2' );
	}
});
...
test( 'dtpttf: N=6, transpose, upper with offset', function t() {
	...
	assert.equal( info, 0, 'info should be 0' );
	for ( i = 0; i < nt; i++ ) {
		assert.equal( actual[ 5 + i ], expected[ i ], 'ARF[' + i + '] mismatch with offset' );
	}
});
