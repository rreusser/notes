...
var zhetrsRook = require( './../lib/ndarray.js' );


// FIXTURES //

var fixtureDir = path.join( __dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' ); // eslint-disable-line max-len
var lines = readFileSync( path.join( fixtureDir, 'zhetrs_rook.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
function runCase( uplo, tc ) {
	...
	info = zhetrsRook( uplo, N, nrhs, A, 1, lda, 0, IPIV, 1, 0, B, 1, lda, 0 );
...
test( 'zhetrs_rook: n0', function t() {
	...
	info = zhetrsRook( 'upper', 0, 1, A, 1, 1, 0, ipiv, 1, 0, b, 1, 1, 0 );
...
test( 'zhetrs_rook: nrhs=0 quick return', function t() {
	...
	info = zhetrsRook( 'lower', 3, 0, A, 1, 3, 0, ipiv, 1, 0, b, 1, 3, 0 );
	assert.equal( info, 0, 'info' );
});
...
test( 'zhetrs_rook: ndarray non-zero offset (upper_4x4)', function t() {
	...
	info = zhetrsRook( 'upper', tc.n, tc.nrhs, Ac, 1, lda, 2, IPIV, 1, 2, Bc, 1, lda, 2 );
	assert.equal( info, tc.info, 'info' );
