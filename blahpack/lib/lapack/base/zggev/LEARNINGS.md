...
- String parameters (`compq`, `compz`, `direct`, `jobvl`, `jobvr`, `side`) use single-char Fortran convention in base.js; ndarray.js normalizes from stdlib full-word strings.
...
- All parameter combinations for `compq`, `compz`, `direct`, `jobvl`, `jobvr`, `side` should be tested to ensure full branch coverage.
