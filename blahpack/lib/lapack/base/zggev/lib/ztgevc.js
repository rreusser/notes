...
*
* All Complex128Array strides and offsets are in complex elements.
* Internally converts to Float64 views for element access.
*
* @private
...
* @param {Complex128Array} S - upper triangular matrix (Schur form of A)
...
* @param {Complex128Array} P - upper triangular matrix (Schur form of B)
...
* @param {Complex128Array} WORK - workspace (length >= 2*N complex elements)
...
function ztgevc( side, howmny, N, S, strideS1, strideS2, offsetS, P, strideP1, strideP2, offsetP, VL, strideVL1, strideVL2, offsetVL, VR, strideVR1, strideVR2, offsetVR, MM, WORK, RWORK ) { // eslint-disable-line max-len, max-params
	...
	var Sv = reinterpret( S, 0 ); // eslint-disable-line no-var
	var Pv = reinterpret( P, 0 ); // eslint-disable-line no-var
	var VLv = reinterpret( VL, 0 ); // eslint-disable-line no-var
	var VRv = reinterpret( VR, 0 ); // eslint-disable-line no-var
	var WORKv = reinterpret( WORK, 0 ); // eslint-disable-line no-var
	...
	for ( j = 1; j < N; j++ ) {
		...
		RWORK[ N + j ] = ZERO;
		for ( i = 0; i < j; i++ ) {
			RWORK[ j ] += abs1( Sv, oS + i * sS1 + j * sS2 );
			RWORK[ N + j ] += abs1( Pv, oP + i * sP1 + j * sP2 );
		}
		anorm = Math.max( anorm, RWORK[ j ] + abs1( Sv, oS + j * sS1 + j * sS2 ) );
		bnorm = Math.max( bnorm, RWORK[ N + j ] + abs1( Pv, oP + j * sP1 + j * sP2 ) );
	}
		...
		for ( je = 0; je < N; je++ ) {
			...
			for ( j = je + 1; j < N; j++ ) {
				...
				for ( jr = je; jr < j; jr++ ) {
					...
					var sr = Sv[ oS + jr * sS1 + j * sS2 ]; // eslint-disable-line no-var
					var si = -Sv[ oS + jr * sS1 + j * sS2 + 1 ]; // eslint-disable-line no-var, max-len
					sumaRe += sr * WORKv[ jr * 2 ] - si * WORKv[ jr * 2 + 1 ];
					sumaIm += sr * WORKv[ jr * 2 + 1 ] + si * WORKv[ jr * 2 ];

					// SUMB += CONJG(P(JR,J)) * WORK(JR)
					var pr = Pv[ oP + jr * sP1 + j * sP2 ]; // eslint-disable-line no-var
					var pi = -Pv[ oP + jr * sP1 + j * sP2 + 1 ]; // eslint-disable-line no-var, max-len
				...
				var sjjr = Sv[ oS + j * sS1 + j * sS2 ]; // eslint-disable-line no-var
				var sjji = Sv[ oS + j * sS1 + j * sS2 + 1 ]; // eslint-disable-line no-var
				var pjjr = Pv[ oP + j * sP1 + j * sP2 ]; // eslint-disable-line no-var
				var pjji = Pv[ oP + j * sP1 + j * sP2 + 1 ]; // eslint-disable-line no-var
	...
	if ( compr ) {
		...
		for ( je = N - 1; je >= 0; je-- ) {
			...
			for ( jr = 0; jr < je; jr++ ) {
				var srr = Sv[ oS + jr * sS1 + je * sS2 ]; // eslint-disable-line no-var
				var sri = Sv[ oS + jr * sS1 + je * sS2 + 1 ]; // eslint-disable-line no-var
				var prr = Pv[ oP + jr * sP1 + je * sP2 ]; // eslint-disable-line no-var
				var pri = Pv[ oP + jr * sP1 + je * sP2 + 1 ]; // eslint-disable-line no-var
			...
			for ( j = je - 1; j >= 0; j-- ) {
				...
				sjjr = Sv[ oS + j * sS1 + j * sS2 ]; // eslint-disable-line no-redeclare
				sjji = Sv[ oS + j * sS1 + j * sS2 + 1 ]; // eslint-disable-line no-redeclare
				pjjr = Pv[ oP + j * sP1 + j * sP2 ]; // eslint-disable-line no-redeclare
				pjji = Pv[ oP + j * sP1 + j * sP2 + 1 ]; // eslint-disable-line no-redeclare
				...
				if ( j > 0 ) {
					...
					for ( jr = 0; jr < j; jr++ ) {
						...
						sr = Sv[ oS + jr * sS1 + j * sS2 ]; // eslint-disable-line no-redeclare
						si = Sv[ oS + jr * sS1 + j * sS2 + 1 ]; // eslint-disable-line no-redeclare
						pr = Pv[ oP + jr * sP1 + j * sP2 ]; // eslint-disable-line no-redeclare
						pi = Pv[ oP + jr * sP1 + j * sP2 + 1 ]; // eslint-disable-line no-redeclare
