...
*/
function dgesvd( jobu, jobvt, M, N, A, strideA1, strideA2, offsetA, s, strideS, offsetS, U, strideU1, strideU2, offsetU, VT, strideVT1, strideVT2, offsetVT ) { // eslint-disable-line max-len, max-params
	var wntua;
	var wntus;
	var wntuas;
	var wntuo;
	...
	anrm = dlange( 'M', M, N, A, sa1, sa2, offsetA, DUM, 1, 0 );
	iscl = 0;
	if ( anrm > 0.0 && anrm < smlnum ) {
		iscl = 1;
		dlascl( 'G', 0, 0, anrm, smlnum, M, N, A, sa1, sa2, offsetA );
	} else if ( anrm > bignum ) {
		iscl = 1;
		dlascl( 'G', 0, 0, anrm, bignum, M, N, A, sa1, sa2, offsetA );
	}
	...
	if ( M >= N ) {
		...
		if ( M >= mnthr ) {
			...
			if ( wntun ) {
				...
				dlacpy( 'U', N, N, A, sa1, sa2, offsetA,
					WK, 1, ldwrkr, ir );
				dlaset( 'L', N - 1, N - 1, 0.0, 0.0,
					WK, 1, ldwrkr, ir + 1 );

				// Generate Q in A
				dorgqr( M, N, N, A, sa1, sa2, offsetA,
					WK, 1, itau, WK, 1, iwork, -1 );
				...
				dorgbr( 'Q', N, N, N, WK, 1, ldwrkr, ir,
					WK, 1, itauq, WK, 1, iwork, -1 );
				iwork = ie + N;

				// Perform bidiagonal QR iteration
				info = dbdsqr( 'U', N, 0, N, 0,
					s, strideS, offsetS,
					WK, 1, ie,
					DUM, 1, 1, 0,                 // VT dummy
				...
				dlacpy( 'L', N, N, VT, svt1, svt2, offsetVT,
					WK, 1, ldwrkr, ir );
				...
				dorgbr( 'P', N, N, N, VT, svt1, svt2, offsetVT,
					WK, 1, itaup, WK, 1, iwork, -1 );
				iwork = ie + N;

				// Perform bidiagonal QR iteration
				info = dbdsqr( 'U', N, N, N, 0,
					s, strideS, offsetS,
					WK, 1, ie,
					VT, svt1, svt2, offsetVT,
					WK, 1, ldwrkr, ir,
				...
				if ( wntvn ) {
					...
					dlacpy( 'U', N, N, WK, 1, ldwrku, iu,
						WK, 1, ldwrkr, ir );

					// Generate left bidiag vectors Q in WORK(IU)
					dorgbr( 'Q', N, N, N, WK, 1, ldwrku, iu,
						WK, 1, itauq, WK, 1, iwork, -1 );

					// Generate right bidiag vectors P^T in WORK(IR)
					dorgbr( 'P', N, N, N, WK, 1, ldwrkr, ir,
						WK, 1, itaup, WK, 1, iwork, -1 );
					iwork = ie + N;
				...
				if ( wntvn ) {
					...
					dlacpy( 'L', M, N, A, sa1, sa2, offsetA,
						U, su1, su2, offsetU );
					...
					dorgqr( M, M, N, U, su1, su2, offsetU,
						WK, 1, itau, WK, 1, iwork, -1 );
			...
			if ( wntuo ) {
				// Generate Q in A
				dorgbr( 'Q', M, N, N, A, sa1, sa2, offsetA,
					WK, 1, itauq, WK, 1, iwork, -1 );
			}
			if ( wntvo ) {
				// Generate P^T in A
				dorgbr( 'P', N, N, N, A, sa1, sa2, offsetA,
					WK, 1, itaup, WK, 1, iwork, -1 );
			}
			iwork = ie + N;
		...
		if ( N >= mnthr ) {
			...
			if ( wntvn ) {
				...
				if ( wntuo || wntuas ) {
					...
					dorgbr( 'Q', M, M, M, A, sa1, sa2, offsetA,
						WK, 1, itauq, WK, 1, iwork, -1 );
				}
				iwork = ie + M;
				nru = 0;
				...
				dlacpy( 'L', M, M, A, sa1, sa2, offsetA,
					WK, 1, ldwrkr, ir );
				dlaset( 'U', M - 1, M - 1, 0.0, 0.0,
					WK, 1, ldwrkr, ir + ldwrkr );

				// Generate Q in A
				dorglq( M, N, M, A, sa1, sa2, offsetA,
					WK, 1, itau, WK, 1, iwork, -1 );
				...
				// Generate right bidiag vectors P^T in WORK(IR)
				dorgbr( 'P', M, M, M, WK, 1, ldwrkr, ir,
					WK, 1, itaup, WK, 1, iwork, -1 );
				iwork = ie + M;

				// Perform bidiagonal QR iteration
				info = dbdsqr( 'U', M, M, 0, 0,
					s, strideS, offsetS,
					WK, 1, ie,
					WK, 1, ldwrkr, ir,
					DUM, 1, 1, 0,
				...
				dlacpy( 'U', M, M, U, su1, su2, offsetU,
					WK, 1, ldwrkr, ir );
				...
				dorgbr( 'Q', M, M, M, U, su1, su2, offsetU,
					WK, 1, itauq, WK, 1, iwork, -1 );
				iwork = ie + M;

				// Perform bidiagonal QR iteration
				info = dbdsqr( 'U', M, M, M, 0,
					s, strideS, offsetS,
					WK, 1, ie,
					WK, 1, ldwrkr, ir,
					U, su1, su2, offsetU,
				...
				if ( wntun ) {
					...
					dlacpy( 'L', M, M, WK, 1, ldwrku, iu,
						WK, 1, ldwrkr, ir );

					// Generate right bidiag vectors P^T in WORK(IU)
					dorgbr( 'P', M, M, M, WK, 1, ldwrku, iu,
						WK, 1, itaup, WK, 1, iwork, -1 );

					// Generate left bidiag vectors Q in WORK(IR)
					dorgbr( 'Q', M, M, M, WK, 1, ldwrkr, ir,
						WK, 1, itauq, WK, 1, iwork, -1 );
					iwork = ie + M;
				...
				if ( wntun ) {
					...
					dlacpy( 'U', M, N, A, sa1, sa2, offsetA,
						VT, svt1, svt2, offsetVT );
					...
					dorglq( N, N, M, VT, svt1, svt2, offsetVT,
						WK, 1, itau, WK, 1, iwork, -1 );
			...
			if ( wntuas ) {
				...
				dlacpy( 'L', M, M, A, sa1, sa2, offsetA,
					U, su1, su2, offsetU );
				dorgbr( 'Q', M, M, N, U, su1, su2, offsetU,
					WK, 1, itauq, WK, 1, iwork, -1 );
			}
			if ( wntvas ) {
				// Copy upper triangle of A to VT, generate P^T
			...
			if ( wntuo ) {
				// Generate Q in A
				dorgbr( 'Q', M, M, N, A, sa1, sa2, offsetA,
					WK, 1, itauq, WK, 1, iwork, -1 );
			}
			if ( wntvo ) {
				// Generate P^T in A
				dorgbr( 'P', M, N, M, A, sa1, sa2, offsetA,
					WK, 1, itaup, WK, 1, iwork, -1 );
			}
			iwork = ie + M;
	...
	if ( iscl === 1 ) {
		if ( anrm > bignum ) {
			dlascl( 'G', 0, 0, bignum, anrm, minmn, 1,
				s, strideS, 1, offsetS );
		}
		...
		if ( anrm < smlnum ) {
			dlascl( 'G', 0, 0, smlnum, anrm, minmn, 1,
				s, strideS, 1, offsetS );
		}
