...
test( 'dtgsy2: notrans_2x2_diag', function t() {
	...
	var rdsum = new Float64Array( 1 );
	var rdscal = new Float64Array( 1 );
	var IWORK = new Int32Array( M + N + 6 );
	var pq = new Int32Array( 1 );

	var info = dtgsy2( 'no-transpose', 0, M, N, A, 1, M, 0, B, 1, N, 0, C, 1, M, 0, D, 1, M, 0, E, 1, N, 0, F, 1, M, 0, scale, rdsum, rdscal, IWORK, 1, 0, pq );
...
test( 'dtgsy2: trans_2x2', function t() {
	...
	var info = dtgsy2( 'transpose', 0, M, N, A, 1, M, 0, B, 1, N, 0, C, 1, M, 0, D, 1, M, 0, E, 1, N, 0, F, 1, M, 0, scale, rdsum, rdscal, IWORK, 1, 0, pq );
...
test( 'dtgsy2: notrans_2x3_bblock', function t() {
	...
	var M = 2;
	var N = 3;
	var A = packMatrix([ 0,0,2.0, 0,1,0.3, 1,1,4.0 ], M, M);
	var B = packMatrix([ 0,0,1.0, 0,1,0.6, 0,2,0.1, 1,0,-0.6, 1,1,1.0, 1,2,0.2, 2,2,3.0 ], N, N);
	var C = new Float64Array([ 1.0, 4.0, 2.0, 5.0, 3.0, 6.0 ]);
	var D = packMatrix([ 0,0,1.0, 0,1,0.1, 1,1,2.0 ], M, M);
	var E = packMatrix([ 0,0,1.0, 0,1,0.3, 0,2,0.1, 1,1,2.0, 1,2,0.2, 2,2,1.5 ], N, N);
	var F = new Float64Array([ 7.0, 10.0, 8.0, 11.0, 9.0, 12.0 ]);
	var scale = new Float64Array( 1 );
	var rdsum = new Float64Array( 1 );
	var rdscal = new Float64Array( 1 );
	var IWORK = new Int32Array( M + N + 6 );
...
test( 'dtgsy2: trans_2x3_bblock', function t() {
	var tc = findCase( 'trans_2x3_bblock' );
	var M = 2;
	var N = 3;
	var A = packMatrix([ 0,0,2.0, 0,1,0.3, 1,1,4.0 ], M, M);
	var B = packMatrix([ 0,0,1.0, 0,1,0.6, 0,2,0.1, 1,0,-0.6, 1,1,1.0, 1,2,0.2, 2,2,3.0 ], N, N);
...
test( 'dtgsy2: trans_3x3_both_quasi', function t() {
	var tc = findCase( 'trans_3x3_both_quasi' );
	var M = 3;
	var N = 3;
...
test( 'dtgsy2: notrans_2x1', function t() {
	var tc = findCase( 'notrans_2x1' );
	var M = 2;
	var N = 1;
	var A = packMatrix([ 0,0,3.0, 0,1,0.5, 1,1,7.0 ], M, M);
	var B = packMatrix([ 0,0,2.0 ], N, N);
	var C = new Float64Array([ 1.0, 4.0 ]);
	var D = packMatrix([ 0,0,1.0, 0,1,0.1, 1,1,1.5 ], M, M);
	var E = packMatrix([ 0,0,1.0 ], N, N);
	var F = new Float64Array([ 5.0, 8.0 ]);
	var scale = new Float64Array( 1 );
	var rdsum = new Float64Array( 1 );
	var rdscal = new Float64Array( 1 );
	var IWORK = new Int32Array( M + N + 6 );
...
test( 'dtgsy2: notrans_1x2', function t() {
	var tc = findCase( 'notrans_1x2' );
	var M = 1;
	var N = 2;
	var A = packMatrix([ 0,0,3.0 ], M, M);
	var B = packMatrix([ 0,0,2.0, 0,1,0.5, 1,1,5.0 ], N, N);
	var C = new Float64Array([ 1.0, 2.0 ]);
	var D = packMatrix([ 0,0,1.5 ], M, M);
	var E = packMatrix([ 0,0,1.0, 0,1,0.1, 1,1,3.0 ], N, N);
	var F = new Float64Array([ 5.0, 6.0 ]);
	var scale = new Float64Array( 1 );
	var rdsum = new Float64Array( 1 );
	var rdscal = new Float64Array( 1 );
	var IWORK = new Int32Array( M + N + 6 );
...
test( 'dtgsy2: ijob=1, 2x2 diagonal (dlatdf path, local look-ahead)', function t() {
	var M = 2;
	var N = 2;
	var A = packMatrix([ 0,0,1.0, 0,1,0.5, 1,1,2.0 ], M, M);
	var B = packMatrix([ 0,0,3.0, 0,1,0.3, 1,1,4.0 ], N, N);
	...
	var info = dtgsy2( 'no-transpose', 1, M, N, A, 1, M, 0, B, 1, N, 0, C, 1, M, 0, D, 1, M, 0, E, 1, N, 0, F, 1, M, 0, scale, rdsum, rdscal, IWORK, 1, 0, pq );

	assert.equal( info, 0, 'info' );
	assert.ok( rdsum[ 0 ] > 0.0, 'rdsum should be updated (got ' + rdsum[ 0 ] + ')' );
	assert.ok( rdscal[ 0 ] > 0.0, 'rdscal should be positive (got ' + rdscal[ 0 ] + ')' );
	assert.ok( pq[ 0 ] > 0, 'pq should be set (got ' + pq[ 0 ] + ')' );
});

test( 'dtgsy2: ijob=2, 2x2 diagonal (dlatdf path, dgecon approximation)', function t() {
	var M = 2;
	var N = 2;
	var A = packMatrix([ 0,0,1.0, 0,1,0.5, 1,1,2.0 ], M, M);
	var B = packMatrix([ 0,0,3.0, 0,1,0.3, 1,1,4.0 ], N, N);
	...
	var info = dtgsy2( 'no-transpose', 2, M, N, A, 1, M, 0, B, 1, N, 0, C, 1, M, 0, D, 1, M, 0, E, 1, N, 0, F, 1, M, 0, scale, rdsum, rdscal, IWORK, 1, 0, pq );
...
test( 'dtgsy2: ijob=1, 3x2 quasi-triangular (dlatdf with 2x1 blocks)', function t() {
	var M = 3;
	var N = 2;
	var A = packMatrix([ 0,0,1.0, 0,1,0.5, 0,2,0.3, 1,0,-0.5, 1,1,1.0, 1,2,0.2, 2,2,3.0 ], M, M);
	var B = packMatrix([ 0,0,2.0, 0,1,0.4, 1,1,5.0 ], N, N);
...
test( 'dtgsy2: ijob=2, 3x3 both quasi-triangular (dlatdf with 2x2 blocks)', function t() {
	var M = 3;
	var N = 3;
	var A = packMatrix([ 0,0,1.0, 0,1,0.4, 0,2,0.1, 1,0,-0.4, 1,1,1.0, 1,2,0.2, 2,2,5.0 ], M, M);
	var B = packMatrix([ 0,0,2.0, 0,1,0.3, 0,2,0.1, 1,0,-0.3, 1,1,2.0, 1,2,0.2, 2,2,6.0 ], N, N);
...
test( 'dtgsy2: ijob=1, 2x3 with B-block (dlatdf with 1x2 blocks)', function t() {
	var M = 2;
	var N = 3;
	var A = packMatrix([ 0,0,2.0, 0,1,0.3, 1,1,4.0 ], M, M);
	var B = packMatrix([ 0,0,1.0, 0,1,0.5, 0,2,0.1, 1,0,-0.5, 1,1,2.0, 1,2,0.3, 2,2,6.0 ], N, N);
	var C = new Float64Array([ 1.0, 3.0, 2.0, 4.0, 5.0, 6.0 ]);
	var D = packMatrix([ 0,0,1.0, 0,1,0.1, 1,1,2.0 ], M, M);
	var E = packMatrix([ 0,0,1.0, 0,1,0.1, 0,2,0.05, 1,1,1.5, 1,2,0.2, 2,2,3.0 ], N, N);
	var F = new Float64Array([ 7.0, 9.0, 8.0, 10.0, 11.0, 12.0 ]);
	var scale = new Float64Array( 1 );
	var rdsum = new Float64Array([ 0.0 ]);
	var rdscal = new Float64Array([ 1.0 ]);
	var IWORK = new Int32Array( M + N + 6 );
