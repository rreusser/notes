...
*/
function zsymm( side, uplo, M, N, alpha, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, beta, C, strideC1, strideC2, offsetC ) {
	var alphaR;
	var alphaI;
	var temp1R;
	var temp1I;
	...
	if ( lside ) {
			...
			for ( j = 0; j < N; j += 1 ) {
				for ( i = 0; i < M; i += 1 ) {
					...
					for ( k = 0; k < i; k += 1 ) {
						...
						// temp2 += B[k,j] * A[k,i] (NO conjugation — symmetric, not Hermitian)
						ib = oB + ( k * sb1 ) + ( j * sb2 );
						bkjR = Bv[ ib ];
						bkjI = Bv[ ib + 1 ];
					...
					// Diagonal of A is complex (not just real — symmetric, not Hermitian)
					ia = oA + ( i * sa1 ) + ( i * sa2 );
					aiiR = Av[ ia ];
					aiiI = Av[ ia + 1 ];
					ic = oC + ( i * sc1 ) + ( j * sc2 );
					...
					// temp1 * A[i,i] (full complex multiply) + alpha * temp2
					if ( betaR === 0.0 && betaI === 0.0 ) {
						Cv[ ic ] = ( temp1R * aiiR ) - ( temp1I * aiiI ) + tR;
						Cv[ ic + 1 ] = ( temp1R * aiiI ) + ( temp1I * aiiR ) + tI;
					} else {
						cR = Cv[ ic ];
						cI = Cv[ ic + 1 ];
						Cv[ ic ] = ( betaR * cR ) - ( betaI * cI ) + ( temp1R * aiiR ) - ( temp1I * aiiI ) + tR;
						Cv[ ic + 1 ] = ( betaR * cI ) + ( betaI * cR ) + ( temp1R * aiiI ) + ( temp1I * aiiR ) + tI;
					}
				}
			}
		} else {
			for ( j = 0; j < N; j += 1 ) {
				for ( i = M - 1; i >= 0; i -= 1 ) {
					...
					for ( k = i + 1; k < M; k += 1 ) {
						...
						// temp2 += B[k,j] * A[k,i] (NO conjugation)
						ib = oB + ( k * sb1 ) + ( j * sb2 );
						bkjR = Bv[ ib ];
						bkjI = Bv[ ib + 1 ];
						temp2R += ( bkjR * akiR ) - ( bkjI * akiI );
		...
		// C := alpha*B*A + beta*C
		for ( j = 0; j < N; j += 1 ) {
			// Diagonal: temp1 = alpha * A[j,j] (full complex multiply)
			ia = oA + ( j * sa1 ) + ( j * sa2 );
			aiiR = Av[ ia ];
			aiiI = Av[ ia + 1 ];
			temp1R = ( alphaR * aiiR ) - ( alphaI * aiiI );
			temp1I = ( alphaR * aiiI ) + ( alphaI * aiiR );
			if ( betaR === 0.0 && betaI === 0.0 ) {
				for ( i = 0; i < M; i += 1 ) {
					ic = oC + ( i * sc1 ) + ( j * sc2 );
					ib = oB + ( i * sb1 ) + ( j * sb2 );
			...
			for ( k = 0; k < j; k += 1 ) {
				...
				// No conjugation — symmetric, not Hermitian
