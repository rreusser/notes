...
test( 'zgemv: basic trans=N (M=2, N=2, alpha=(1,0), beta=(0,0))', function t() {
	...
	assertArrayClose( Array.from( y ), tc.y, 'zgemv_basic y' );
});
...
test( 'zgemv: conjugate transpose (trans=C, M=2, N=2)', function t() {
	...
	assert.strictEqual( result, y );
	assertArrayClose( Array.from( y ), tc.y, 'zgemv_conj_trans y' );
});
...
test( 'zgemv: alpha and beta scaling (trans=N)', function t() {
	...
	var result = base( 'N', 2, 2, alpha, A, 1, 2, 0, x, 1, 0, beta, y, 1, 0 );
	assert.strictEqual( result, y );
	assertArrayClose( Array.from( y ), tc.y, 'zgemv_alpha_beta y' );
});
...
test( 'zgemv: non-unit stride (incx=2, incy=2, trans=N)', function t() {
	...
	var x = new Float64Array( [ 1, 1, 99, 99, 2, 2 ] );
	// y: element 0=(0,0), gap=(88,88), element 1=(0,0)
	var y = new Float64Array( [ 0, 0, 88, 88, 0, 0 ] );
	var alpha = new Float64Array( [ 1, 0 ] );
	var beta = new Float64Array( [ 0, 0 ] );
	var result = base( 'N', 2, 2, alpha, A, 1, 2, 0, x, 2, 0, beta, y, 2, 0 );
	assert.strictEqual( result, y );
	assertArrayClose( Array.from( y ), tc.y, 'zgemv_stride y' );
});
