...
var require_base3 = __commonJS({
  "lib/lapack/base/dlartg/lib/base.js"(exports, module) {
    ...
    var RTMIN = Math.sqrt(SAFMIN2);
    var RTMAX = Math.sqrt(SAFMAX2 / 2);
...
var require_main62 = __commonJS({
  "node_modules/@stdlib/math/base/ops/csub/lib/main.js"(exports, module) {
    ...
    var real = require_lib54();
    var imag = require_lib55();
    function csub4(z1, z2) {
      var re = real(z1) - real(z2);
      var im = imag(z1) - imag(z2);
      return new Complex128(re, im);
    }
    module.exports = csub4;
  }
});
...
// examples/stokes/stokes.js
var csub2 = (a, b) => [a[0] - b[0], a[1] - b[1]];
...
function stokes(options = {}) {
  const {
    ...
    boundaryBC = ([x, y]) => {
      if (Math.abs(y - 1) < 1e-10) return { psi: 0, tangent: 1, tangentDir: "u" };
      if (Math.abs(y + 1) < 1e-10) return { psi: 0, tangent: 0, tangentDir: "u" };
      if (Math.abs(x - 1) < 1e-10) return { psi: 0, tangent: 0, tangentDir: "v" };
      if (Math.abs(x + 1) < 1e-10) return { psi: 0, tangent: 0, tangentDir: "v" };
  ...
  return {
    ...
    evaluate,
    va
    // Hessenberg data for GPU evaluation
  };
}
...
function laplace(boundaryPoints, boundaryValues, polygonVertices, options = {}) {
  ...
  for (let j = 0; j < M; j++) {
    let base;
    if (interior) {
      base = csub3(boundaryPoints[j], c);
    } else {
      const dz = csub3(boundaryPoints[j], c);
      const d = dz[0] * dz[0] + dz[1] * dz[1];
      base = [dz[0] / d, -dz[1] / d];
    }
    let power = [1, 0];
  ...
  for (let j = 0; j < M; j++) {
    for (let k = 0; k < K; k++) {
      const dz = csub3(boundaryPoints[j], filteredPoles[k]);
      const denom = dz[0] * dz[0] + dz[1] * dz[1];
