...
function squareFlow(options = {}) {
  ...
  const monoCoeffs = new Float64Array(2 * nPolyBasis);
  for (let n = 0; n < nPolyBasis; n++) {
...
function csub2(out, a, b) {
  out[0] = a[0] - b[0];
  out[1] = a[1] - b[1];
  return out;
}
function cmul2(out, a, b) {
  ...
  for (let jiter = 1; jiter <= maxit; jiter++) {
    ...
    if (converged) {
      ...
      if (absb > safmin) {
        ...
        cmul2(H[ilast][ilast], _t, H[ilast][ilast]);
      } else {
        cset(T[ilast][ilast], 0, 0);
      }
