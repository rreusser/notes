program test_dsyswapr
  ...
  double precision :: A0(LDA*N)
  integer :: i, j

  ! Build a symmetric base matrix, stored fully (both triangles),
  ! so we can reuse it for both UPLO='U' and UPLO='L' tests.
  ! A(i,j) = 10*min(i,j) + max(i,j)  (symmetric, distinct-valued)
  do j = 1, N
    do i = 1, N
      A0((j-1)*LDA + i) = dble( 10 * min(i,j) + max(i,j) )
    end do
  end do
