program test_zpteqr
  ...
  double precision :: D(MAXN), E(MAXN), WORK(4*MAXN)
  complex*16 :: Z(LDZ, MAXN), ZTMP(MAXN*MAXN)
  double precision :: ZTMP_r(2*MAXN*MAXN)
  equivalence (ZTMP, ZTMP_r)
  integer :: INFO, i, j, N
  ...
  call ZPTEQR('N', N, D, E, Z, LDZ, WORK, INFO)
  ...
  D(1) = 4.0d0; D(2) = 4.0d0; D(3) = 4.0d0; D(4) = 4.0d0
  E(1) = 1.0d0; E(2) = 1.0d0; E(3) = 1.0d0
  call ZPTEQR('I', N, D, E, Z, LDZ, WORK, INFO)
  ! Extract NxN block from LDZ-padded Z into ZTMP
  do j = 1, N
    do i = 1, N
      ZTMP((j-1)*N + i) = Z(i, j)
    end do
  ...
  call print_int('info', INFO)
  call print_array('d', D, N)
  call print_array('z', ZTMP_r, 2*N*N)
  call end_test()
  ...
  call ZPTEQR('V', N, D, E, Z, LDZ, WORK, INFO)
  do j = 1, N
    do i = 1, N
      ZTMP((j-1)*N + i) = Z(i, j)
    end do
