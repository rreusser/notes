...
def parse_loop_control(ctrl):
    ...
    if ctrl.items[0] is not None and ctrl.items[1] is None and ctrl.items[2] is None:
        return {
            'type': 'WhileStatement',
            'test': to_estree(ctrl.items[0]),
            'body': None
        }
    else:
        var = to_estree(ctrl.items[1][0])
        rng = ctrl.items[1][1]
        start = to_estree(rng[0])
        end = to_estree(rng[1])
        try:
            inc = ctrl.items[1][2]
        except IndexError:
...
def collect_specification_vars (specification):
...
def to_estree (node):
    if isinstance(node, list):
        return filt([to_estree(child) for child in node])
        ...
        return {
            'type': 'BinaryExpression',
            'left': to_estree(node.items[0]),
            'operator': LEVEL_2_OPERATORS[node.items[1]],
            'right': to_estree(node.items[2])
        }
        ...
        if get_child(node, Nonlabel_Do_Stmt):
            ...
            expr = parse_loop_control(loop_control)
            expr['body'] = block(to_estree(node.children[nonlabel_do_stmt_idx + 1: end_do_stmt_idx]))
            return expr
        else:
        ...
        return {
            ...
            'computed': True,
            'object': to_estree(name),
            'property': to_estree(subs.items[0])
        }

    elif isinstance(node, Execution_Part):
        return to_estree(node.children)
        ...
        vars = flatten([collect_specification_vars(spec) for spec in specs])
        ...
        body.extend(to_estree(execution_part))

        return {
            "type": "FunctionDeclaration",
            "id": to_estree(function_stmt.get_name()),
            "params": filt([{
                "type": "Identifier",
                "name": arg.string
        ...
        return {
            'type': 'IfStatement',
            'test': to_estree(node.items[0]),
            'consequent': block(to_estree(node.items[1])),
            'alternate': None
        }
        ...
        if get_child(node, If_Then_Stmt):
            ...
            for i in range(len(indices) - 1):
                ...
                if isinstance(ctrl_node, If_Then_Stmt):
                    ...
                    tree = {
                        'type': 'IfStatement',
                        'test': to_estree(ctrl_node.items[0]),
                        'consequent': block(to_estree(node.children[start_idx + 1: end_idx])),
                        'alternate': None,
                    }
                    cur_tree = tree
                    ...
                    assert cur_tree is not None
                    cur_tree['alternate'] = block(to_estree(node.children[start_idx + 1: end_idx]))
                else:
        ...
        if func in INTRINSIC_FUNCTIONS:
            return {
                ...
                'arguments': filt([to_estree(arg) for arg in args.items])
            }
        else:
        ...
        assert node.items[0] == '('
        assert node.items[2] == ')'
        return to_estree(node.items[1])
    
    elif isinstance(node, Program):
        return {
            'type': 'Program',
            'body': to_estree(node.children)
        }
        ...
        return {
            "type": "FunctionDeclaration",
            "id": to_estree(subroutine_stmt.get_name()),
            "params": filt([{
                "type": "Identifier",
                "name": arg.string
            } for arg in dummy_arg_list.items]),
        ...
        return {
            ...
            'argument': to_estree(node.items[1]),
            'prefix': True
        }
...
def find_all(node, node_type):
    return [node for node in walk(ast) if isinstance(node, node_type)]

def process_subroutine(subroutine):
    specs = find_all(subroutine, Specification_Part)
    vars = flatten([collect_specification_vars(spec) for spec in specs])

    return to_estree(subroutine)

# Define your Fortran source code
with open(sys.argv[1], 'rt') as f:
    fortran_code = f.read()

# Parse the Fortran code into an AST
reader = FortranStringReader(fortran_code, ignore_comments=False)
parser = ParserFactory().create(std="f2003")
ast = parser(reader)

print(json.dumps( to_estree(ast), indent=2 ))
