...
var lines = readFileSync( path.join( fixtureDir, 'dorbdb4.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
// Match NMAX in test_dorbdb4.f90 — leading dimension of the X11/X21 buffers
var LDX_PAD = 16;
...
function runFixtureCase( name, M, P, Q, hasPhi, hasPhantom ) {
	...
	PHANTOM = new Float64Array( 2 * LDX_PAD );
	WORK = new Float64Array( LDX_PAD * LDX_PAD );

	info = dorbdb4( M, P, Q, X11, 1, LDX_PAD, 0, X21, 1, LDX_PAD, 0, THETA, 1, 0, PHI, 1, 0, TAUP1, 1, 0, TAUP2, 1, 0, TAUQ1, 1, 0, PHANTOM, 1, 0, WORK, 1, 0 );
	...
	if ( hasPhantom ) {
		assertArrayClose( TAUP1.subarray( 0, M - Q ), tc.TAUP1, 1e-12, name + ': TAUP1' );
		assertArrayClose( TAUP2.subarray( 0, M - Q ), tc.TAUP2, 1e-12, name + ': TAUP2' );
		assertArrayClose( PHANTOM.subarray( 0, M ), tc.PHANTOM, 1e-12, name + ': PHANTOM' );
	}
}
...
test( 'dorbdb4.ndarray: basic_8x4x6 (all three loops exercised)', function t() {
	runFixtureCase( 'basic_8x4x6', 8, 4, 6, true, true );
});

test( 'dorbdb4.ndarray: m10_p5_q7 (larger general case)', function t() {
	runFixtureCase( 'm10_p5_q7', 10, 5, 7, true, true );
});

test( 'dorbdb4.ndarray: p_eq_q_no_third (third cleanup loop empty when P=Q)', function t() {
	runFixtureCase( 'p_eq_q_no_third', 8, 6, 6, true, true );
});

test( 'dorbdb4.ndarray: p_small_q_large (third cleanup loop wider than second)', function t() {
	runFixtureCase( 'p_small_q_large', 10, 4, 8, true, true );
});

test( 'dorbdb4.ndarray: m_eq_q_no_phantom (M=Q so phantom outer loop is skipped)', function t() {
	runFixtureCase( 'm_eq_q_no_phantom', 4, 2, 4, true, false );
});

test( 'dorbdb4.ndarray: throws RangeError for negative M', function t() {
	var X = new Float64Array( 1 );
	assert.throws( function throws() {
		dorbdb4( -1, 0, 0, X, 1, 1, 0, X, 1, 1, 0, X, 1, 0, X, 1, 0, X, 1, 0, X, 1, 0, X, 1, 0, X, 1, 0, X, 1, 0 );
	}, RangeError );
});

test( 'dorbdb4.ndarray: throws RangeError when P < M-Q', function t() {
	var X = new Float64Array( 16 );

	// M=8, Q=4, M-Q=4, P=2 violates P >= M-Q
	assert.throws( function throws() {
		dorbdb4( 8, 2, 4, X, 1, 4, 0, X, 1, 4, 0, X, 1, 0, X, 1, 0, X, 1, 0, X, 1, 0, X, 1, 0, X, 1, 0, X, 1, 0 );
	}, RangeError );
});

test( 'dorbdb4.ndarray: throws RangeError when M-Q > Q', function t() {
	var X = new Float64Array( 16 );

	// M=4, Q=1, M-Q=3 > Q=1 violates M-Q <= Q
	assert.throws( function throws() {
		dorbdb4( 4, 2, 1, X, 1, 4, 0, X, 1, 4, 0, X, 1, 0, X, 1, 0, X, 1, 0, X, 1, 0, X, 1, 0, X, 1, 0, X, 1, 0 );
	}, RangeError );
});

test( 'dorbdb4.ndarray: throws RangeError when Q > M', function t() {
	var X = new Float64Array( 16 );

	// M=4, P=4, Q=5: Q > M violates Q <= M
	assert.throws( function throws() {
		dorbdb4( 4, 4, 5, X, 1, 4, 0, X, 1, 4, 0, X, 1, 0, X, 1, 0, X, 1, 0, X, 1, 0, X, 1, 0, X, 1, 0, X, 1, 0 );
	}, RangeError );
});

test( 'dorbdb4.ndarray: returns info=0 when called with M=Q=0 (degenerate)', function t() {
	var PHANTOM;
	var THETA;
	var TAUP1;
	var TAUP2;
	...
	PHANTOM = new Float64Array( 1 );
	WORK = new Float64Array( 1 );

	info = dorbdb4( 0, 0, 0, X11, 1, 1, 0, X21, 1, 1, 0, THETA, 1, 0, PHI, 1, 0, TAUP1, 1, 0, TAUP2, 1, 0, TAUQ1, 1, 0, PHANTOM, 1, 0, WORK, 1, 0 );
