...
var lines = readFileSync( path.join( fixtureDir, 'zsytri_3x.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
function runFixture( uplo, N, nb, tc ) {
	...
	info = zsytri3x( uplo, N, A, 1, LDA, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0, nb );
...
test( 'zsytri_3x: 4x4 upper definite, nb=2', function t() {
	runFixture( 'upper', 4, 2, findCase( '4x4_upper_def_nb2' ) );
});

test( 'zsytri_3x: 4x4 lower definite, nb=2', function t() {
	runFixture( 'lower', 4, 2, findCase( '4x4_lower_def_nb2' ) );
});

test( 'zsytri_3x: 4x4 upper indefinite (2x2 pivots), nb=2', function t() {
	runFixture( 'upper', 4, 2, findCase( '4x4_upper_indef_nb2' ) );
});

test( 'zsytri_3x: 4x4 lower indefinite (2x2 pivots), nb=2', function t() {
	runFixture( 'lower', 4, 2, findCase( '4x4_lower_indef_nb2' ) );
});

test( 'zsytri_3x: 5x5 lower mixed pivots, nb=2', function t() {
	runFixture( 'lower', 5, 2, findCase( '5x5_lower_mixed_nb2' ) );
});

test( 'zsytri_3x: 5x5 upper mixed pivots, nb=3', function t() {
	runFixture( 'upper', 5, 3, findCase( '5x5_upper_mixed_nb3' ) );
});

test( 'zsytri_3x: N=1 lower trivial inverse', function t() {
	var ldwork;
	var Aview;
	var ipiv;
	var work;
	...
	info = zsytri3x( 'lower', 1, A, 1, LDA, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0, 1 );
	assert.equal( info, tc.info, 'info' );
	Aview = new Float64Array( A.buffer );
...
test( 'zsytri_3x: N=0 quick return', function t() {
	var ipiv;
	var work;
	var info;
	var A;
	...
	info = zsytri3x( 'lower', 0, A, 1, 1, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0, 1 );
	assert.equal( info, 0, 'info' );
});

test( 'zsytri_3x: validation throws on invalid uplo', function t() {
	var ipiv;
	var work;
	var A;
	var e;
	...
	assert.throws( function fn() {
		zsytri3x( 'invalid', 2, A, 1, 2, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0, 1 );
	}, TypeError );
});
