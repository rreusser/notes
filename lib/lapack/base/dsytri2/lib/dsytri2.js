...
/**
* Computes the inverse of a real symmetric indefinite matrix using the factorization produced by `dsytrf`. Internally dispatches between the unblocked `dsytri` (when `N <= 32`) and the blocked `dsytri2x` (when `N > 32`).
*
...
*/
function dsytri2( order, uplo, N, A, LDA, IPIV ) {
	var ldwork;
	var WORK;
	var sa1;
	var sa2;
	...
	if ( NBMAX >= N ) {
		...
		ldwork = N + NBMAX + 1;
		WORK = new Float64Array( ldwork * ( NBMAX + 3 ) );
	}
