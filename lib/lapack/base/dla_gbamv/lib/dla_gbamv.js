...
*/
function dla_gbamv( order, trans, M, N, kl, ku, alpha, AB, LDAB, x, strideX, beta, y, strideY ) {
	var sab1;
	var sab2;
	var lenx;
	var leny;
	...
	if ( LDAB < kl + ku + 1 ) {
		throw new RangeError( format( 'invalid argument. Ninth argument must be greater than or equal to `kl+ku+1`. Value: `%d`.', LDAB ) );
	}
	if ( strideX === 0 ) {
