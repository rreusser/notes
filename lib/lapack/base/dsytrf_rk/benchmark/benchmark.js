...
function createBenchmark( N ) {
	var Aref = uniform( N * N, -1.0, 1.0, options );
	var i;

	// Make A symmetric (lower) and diagonally dominant for stability:
	for ( i = 0; i < N; i++ ) {
		Aref[ ( i * N ) + i ] = N + 1.0;
	}
	return benchmark;
	...
	*/
	function benchmark( b ) {
		...
		for ( k = 0; k < b.iterations; k++ ) {
			A = new Float64Array( Aref );
			e = new Float64Array( N );
			IPIV = new Int32Array( N );
			y = dsytrfrk( 'column-major', 'lower', N, A, N, e, 1, IPIV, 1 );
			if ( isnan( y ) ) {
