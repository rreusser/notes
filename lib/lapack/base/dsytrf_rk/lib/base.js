...
var dsytf2rk = require( '../../dsytf2_rk/lib/base.js' );
var dlasyfrk = require( '../../dlasyf_rk/lib/base.js' );
...
*
* This is the `_rk` driver: the diagonal of `D` is stored on the diagonal of `A` while the off-diagonal entries of `D` are returned separately in `e`. The array `e` is zero on entries corresponding to 1x1 pivot blocks.
*
...
*/
function dsytrfrk( uplo, N, A, strideA1, strideA2, offsetA, e, strideE, offsetE, IPIV, strideIPIV, offsetIPIV ) {
	var ldwork;
	var result;
	var iinfo;
	var ipidx;
	...
	if ( uplo === 'upper' ) {
		...
		while ( k >= 1 ) {
			if ( k > nb ) {
				...
				result = dlasyfrk( 'upper', k, nb, A, strideA1, strideA2, offsetA, e, strideE, offsetE, IPIV, strideIPIV, offsetIPIV, W, 1, ldwork, 0 );
				kb = result.kb;
				iinfo = result.info;
			} else {
				// Use the unblocked kernel for the remaining leading submatrix
				iinfo = dsytf2rk( 'upper', k, A, strideA1, strideA2, offsetA, e, strideE, offsetE, IPIV, strideIPIV, offsetIPIV );
				kb = k;
			}
			...
			if ( k < N ) {
				for ( i = k; i >= ( k - kb + 1 ); i -= 1 ) {
					...
					if ( ip !== ipidx ) {
						...
						dswap( N - k, A, strideA2, offsetA + ( ipidx * strideA1 ) + ( k * strideA2 ), A, strideA2, offsetA + ( ip * strideA1 ) + ( k * strideA2 ) );
					}
				}
			}
		...
		while ( k <= N ) {
			if ( k <= N - nb ) {
				...
				result = dlasyfrk( 'lower', N - k + 1, nb, A, strideA1, strideA2, offsetA + ( ( k - 1 ) * strideA1 ) + ( ( k - 1 ) * strideA2 ), e, strideE, offsetE + ( ( k - 1 ) * strideE ), IPIV, strideIPIV, offsetIPIV + ( ( k - 1 ) * strideIPIV ), W, 1, ldwork, 0 );
				kb = result.kb;
				iinfo = result.info;
			} else {
				// Use the unblocked kernel for the trailing submatrix
				iinfo = dsytf2rk( 'lower', N - k + 1, A, strideA1, strideA2, offsetA + ( ( k - 1 ) * strideA1 ) + ( ( k - 1 ) * strideA2 ), e, strideE, offsetE + ( ( k - 1 ) * strideE ), IPIV, strideIPIV, offsetIPIV + ( ( k - 1 ) * strideIPIV ) );
				kb = N - k + 1;
			}
			...
			if ( k > 1 ) {
				for ( i = k; i <= k + kb - 1; i += 1 ) {
					...
					if ( ip !== ipidx ) {
						...
						dswap( k - 1, A, strideA2, offsetA + ( ipidx * strideA1 ), A, strideA2, offsetA + ( ip * strideA1 ) );
					}
				}
			}
