...
var dgeqr2p = require( './../../dgeqr2p/lib/base.js' );
...
* `N*NB + NB*NB` elements (with `NB = 32`) when the blocked path is
* taken (i.e., `min(M,N) > NB`). The buffer is partitioned so that
...
*/
function dgeqrfp( M, N, A, strideA1, strideA2, offsetA, TAU, strideTAU, offsetTAU, WORK, strideWORK, offsetWORK ) {
	var offsetT;
	var ldwork;
	var nbmin;
	var iws;
	...
	if ( nb >= nbmin && nb < K && nx < K ) {
		...
		while ( i <= K - 1 - nx ) {
			...
			dgeqr2p( M - i, ib, A, strideA1, strideA2, offsetA + ( i * strideA1 ) + ( i * strideA2 ), TAU, strideTAU, offsetTAU + ( i * strideTAU ), WORK, strideWORK, offsetWORK );
	...
	if ( i <= K - 1 ) {
		dgeqr2p( M - i, N - i, A, strideA1, strideA2, offsetA + ( i * strideA1 ) + ( i * strideA2 ), TAU, strideTAU, offsetTAU + ( i * strideTAU ), WORK, strideWORK, offsetWORK );
	}
