...
test( 'ztpmlqt: WORK=null triggers internal allocation', function t() {
	var inputs = initLeftA();
	var info;
	var vt;
	var tc;
	vt = loadVT( 'factors_left_A' );
	info = ztpmlqt( 'left', 'no-transpose', 4, 3, 3, 2, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 4, 0, null, 1, 0 );
	tc = findCase( 'left_notrans_A' );
	assert.equal( info, 0, 'info' );
	assertArrayClose( reinterpret( inputs.A, 0 ), tc.A, 1e-12, 'A' );
	assertArrayClose( reinterpret( inputs.B, 0 ), tc.B, 1e-12, 'B' );
...
test( 'ztpmlqt: undersized WORK triggers internal allocation (left side)', function t() {
	var inputs = initLeftA();
...
test( 'ztpmlqt: undersized WORK triggers internal allocation (right side)', function t() {
	var inputs = initRightB();
	var WORK = new Complex128Array( 1 );
	var info;
	var vt;
