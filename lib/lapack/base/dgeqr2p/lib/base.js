...
/**
* Computes a QR factorization of a real M-by-N matrix A = Q * R with non-negative diagonal elements (unblocked algorithm).
*
* @private
...
*/
function dgeqr2p( M, N, A, strideA1, strideA2, offsetA, TAU, strideTAU, offsetTAU, WORK, strideWORK, offsetWORK ) {
	var alpha;
	var aii;
	var K;
	var i;
	...
	for ( i = 0; i < K; i++ ) {
		...
		dlarfgp( M - i, A, aii, A, strideA1, offsetA + (Math.min( i + 1, M - 1 ) * strideA1) + (i * strideA2), TAU, offsetTAU + (i * strideTAU) );
