...
/**
* Interface describing `dtpmqrt`.
*/
interface Routine {
	/**
	* Apply Q from a triangular-pentagonal QR factorization (output of dtpqrt) to a stacked matrix C composed of A and B blocks
	*
	...
	* @returns result
	*/
	( order: Layout, side: string, trans: TransposeOperation, M: number, N: number, K: number, l: number, nb: number, V: Float64Array, LDV: number, T: Float64Array, LDT: number, A: Float64Array, LDA: number, B: Float64Array, LDB: number, WORK: Float64Array, strideWORK: number ): Float64Array;

	/**
	* Apply Q from a triangular-pentagonal QR factorization (output of dtpqrt) to a stacked matrix C composed of A and B blocks, using alternative indexing semantics.
	*
	* @param side - specifies the operation type
	* @param trans - specifies the operation type
	* @param M - number of rows
