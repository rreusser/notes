...
* @param {Float64Array} WORK - workspace buffer (logically `nb`-by-`N` for `side='left'` or `M`-by-`nb` for `side='right'`)
...
function dtpmqrt( side, trans, M, N, K, l, nb, V, strideV1, strideV2, offsetV, T, strideT1, strideT2, offsetT, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, WORK, strideWORK, offsetWORK ) {
	...
	// Workspace size matches the Fortran convention: N*NB doubles for SIDE='L' (logical layout NB-by-N, leading dim = IB, but the maximum occurs at IB = NB) and M*NB for SIDE='R' (logical layout M-by-NB). We treat WORK as ldwork-by-ncwork laid out with the user's stride; dtprfb needs separate first/second-dim strides. The buffer is contiguous along the leading dimension (stride1 = strideWORK) and jumps `ldwork * strideWORK` between columns.
	if ( left ) {
		ldwork = nb;
		ncwork = N;
	} else {
