...
var zlarzb = require( './../lib/base.js' );
...
test( 'zlarzb: unsupported direct returns without error', function t() {
	var WORK;
	var Cv;
	var V;
	var T;
	...
	assert.equal( Cv[ 0 ], 5.0, 'C unchanged for unsupported direct' );
});

test( 'zlarzb: unsupported storev returns without error', function t() {
	var WORK;
	var Cv;
	var V;
	var T;
	...
	assert.equal( Cv[ 0 ], 6.0, 'C unchanged for unsupported storev' );
});
