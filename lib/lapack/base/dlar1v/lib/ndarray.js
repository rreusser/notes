...
* @param {Float64Array} ztz - output (length >= 1): 2-norm squared of `Z`
* @param {Float64Array} mingma - output (length >= 1): minimum gamma value
* @param {Int32Array} r - in/out (length >= 1): twist index (0 on input = select)
* @param {Int32Array} ISUPPZ - output (length >= 2): 1-based support indices
...
* @param {Float64Array} resid - output (length >= 1): residual
* @param {Float64Array} rqcorr - output (length >= 1): Rayleigh quotient correction
* @param {Float64Array} WORK - workspace (length >= 4*N)
* @param {integer} strideWORK - stride length for `WORK`
* @param {NonNegativeInteger} offsetWORK - starting index for `WORK`
* @throws {TypeError} arguments must have the correct type
* @throws {RangeError} arguments must satisfy numeric constraints
*/
function dlar1v( N, b1, bn, lambda, D, strideD, offsetD, L, strideL, offsetL, LD, strideLD, offsetLD, LLD, strideLLD, offsetLLD, pivmin, gaptol, Z, strideZ, offsetZ, wantnc, negcnt, ztz, mingma, r, ISUPPZ, strideISUPPZ, offsetISUPPZ, nrminv, resid, rqcorr, WORK, strideWORK, offsetWORK ) {
	...
	if ( !isFloat64Array( Z ) ) {
		throw new TypeError( format( 'invalid argument. 19th argument must be a Float64Array. Value: `%s`.', Z ) );
	}
