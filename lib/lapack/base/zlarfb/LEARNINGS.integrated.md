...
- String parameters (`direct`, `side`, `storev`, `trans`, `transt`) use single-char Fortran convention in base.js; ndarray.js normalizes from stdlib full-word strings.
...
- Dependencies: `reinterpret-complex128`, `zcopy`, `zgemm`, `ztrmm`. No unexpected interface issues encountered.
...
- All parameter combinations for `direct`, `side`, `storev`, `trans`, `transt` should be tested to ensure full branch coverage.
