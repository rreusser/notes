 ...
 * @param {Complex128Array} WORK - caller-provided workspace; size must be at least `(ldwork * nb) + (ldt * nb)` complex elements, where `nb = min(32, K)`, `ldt = nb + 1`, and `ldwork = max(1, N)` for `side = 'left'` or `max(1, M)` for `side = 'right'`.
