...
> Simultaneously bidiagonalize the blocks of a tall and skinny matrix `[X11; X21]` with orthonormal columns. Used as a building block in the CS Decomposition (CSD).
...
#### dorbdb1( order, M, P, Q, X11, LDX11, X21, LDX21, THETA, strideTHETA, PHI, stridePHI, TAUP1, strideTAUP1, TAUP2, strideTAUP2, TAUQ1, strideTAUQ1, WORK, strideWORK )
...
var X21 = new Float64Array( [ sq2, 0.0, 0.0, sq2 ] );
...
-   **WORK**: workspace of length at least `M-Q` ([`Float64Array`][mdn-float64array]).
-   **strideWORK**: stride length for `WORK`.

#### dorbdb1.ndarray( M, P, Q, X11, strideX111, strideX112, offsetX11, X21, strideX211, strideX212, offsetX21, THETA, strideTHETA, offsetTHETA, PHI, stridePHI, offsetPHI, TAUP1, strideTAUP1, offsetTAUP1, TAUP2, strideTAUP2, offsetTAUP2, TAUQ1, strideTAUQ1, offsetTAUQ1, WORK, strideWORK, offsetWORK )

Same as `dorbdb1` above, but with explicit per-array stride and offset parameters in place of the `order`/`LDX` layout convention.
...
dorbdb1.ndarray( 8, 4, 0, X11, 1, 4, 0, X21, 1, 4, 0, THETA, 1, 0, PHI, 1, 0, TAUP1, 1, 0, TAUP2, 1, 0, TAUQ1, 1, 0, WORK, 1, 0 );
```
...
-   `dorbdb1` is one of four sister routines (`dorbdb1`/`dorbdb2`/`dorbdb3`/`dorbdb4`) that handle the four orientations of the input matrix in CS-decomposition reductions. `dorbdb1` is the variant for which `Q <= min(P, M-P, M-Q)` (i.e. the input is "tall enough" in both block directions).
