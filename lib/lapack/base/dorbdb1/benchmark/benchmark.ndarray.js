...
/**
* Build orthonormal-column inputs of size M-by-Q with `Q = M/4` (so the dimension constraints are easily satisfied at `P = M/2`).
*
* @private
...
function makeInputs( M ) {
	...
	return {
		...
		'TAUP2': new Float64Array( M - P ),
		'TAUQ1': new Float64Array( Q ),
		'WORK': new Float64Array( M - Q )
	};
}
	...
	*/
	function benchmark( b ) {
		...
		for ( i = 0; i < b.iterations; i++ ) {
			...
			info = dorbdb1( M, ins.P, ins.Q, X11, 1, ins.P, 0, X21, 1, M - ins.P, 0, ins.THETA, 1, 0, ins.PHI, 1, 0, ins.TAUP1, 1, 0, ins.TAUP2, 1, 0, ins.TAUQ1, 1, 0, ins.WORK, 1, 0 );
			if ( info !== 0 ) {
