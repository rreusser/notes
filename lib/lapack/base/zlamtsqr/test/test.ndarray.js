...
test( 'zlamtsqr WORK undersize falls back to internal allocation (forwarded)', function t() {
	var WORK;
	var info;
	var view;
	var tc;
	...
	// Reuse a blocked fixture but pass an empty WORK.
	tc = findCase( 'left_notrans_blocked' );
	A = makeComplex( tc.A );
	T = makeComplex( tc.T );
	C = makeComplex( tc.C0 );
	WORK = new Complex128Array( 1 ); // intentionally too small

	info = zlamtsqr( 'left', 'no-transpose', tc.M, tc.N, tc.K, tc.MB, tc.NB, A, 1, tc.M, 0, T, 1, tc.LDT, 0, C, 1, tc.M, 0, WORK, 1, 0, WORK.length );
	assert.strictEqual( info, 0, 'info 0' );
	view = reinterpret( C, 0 );
	assertArrayClose( view, tc.C, 1e-12, 'C (undersized WORK)' );
});
