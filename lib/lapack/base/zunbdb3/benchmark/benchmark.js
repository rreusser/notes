...
/**
* Builds complex orthonormal-column inputs of size `M`-by-`Q` with `P = 3*M/4` and `Q = M/2`. The zunbdb3 partition requires `M-P <= min(P, Q, M-Q)`, here `M/4 <= min(3M/4, M/2, M/2) = M/2`.
*
* @private
* @param {PositiveInteger} M - total number of rows
* @returns {Object} packed inputs (`X11`, `X21`, `THETA`, `PHI`, `TAU*`, `WORK`, `P`, `Q`, `LD`)
*/
function makeInputs( M ) {
	...
	// Allocate X11 and X21 with matching leading dimension `M` so that ZDROT in zunbdb3 (which uses LDX11 for both X11 and X21 row strides — see LEARNINGS.md) operates correctly.
	X11 = new Complex128Array( M * Q );
	X21 = new Complex128Array( M * Q );
	for ( j = 0; j < Q; j++ ) {
		for ( i = 0; i < P; i++ ) {
			reinterpret( X11, 0 )[ 2 * ( i + ( j * M ) ) ] = av[ 2 * ( i + ( j * M ) ) ];
			reinterpret( X11, 0 )[ ( 2 * ( i + ( j * M ) ) ) + 1 ] = av[ ( 2 * ( i + ( j * M ) ) ) + 1 ];
		}
		for ( i = 0; i < M - P; i++ ) {
			reinterpret( X21, 0 )[ 2 * ( i + ( j * M ) ) ] = av[ 2 * ( ( P + i ) + ( j * M ) ) ];
			reinterpret( X21, 0 )[ ( 2 * ( i + ( j * M ) ) ) + 1 ] = av[ ( 2 * ( ( P + i ) + ( j * M ) ) ) + 1 ];
		}
	}
	return {
		'P': P,
	...
	*/
	function benchmark( b ) {
		...
		for ( i = 0; i < b.iterations; i++ ) {
			...
			info = zunbdb3( 'column-major', M, ins.P, ins.Q, X11, ins.LD, X21, ins.LD, ins.THETA, 1, ins.PHI, 1, ins.TAUP1, 1, ins.TAUP2, 1, ins.TAUQ1, 1, ins.WORK, 1 );
			if ( info !== 0 ) {
