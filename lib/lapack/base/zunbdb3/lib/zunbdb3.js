...
* @param {Complex128Array} TAUP1 - output array (length at least `P`)
* @param {integer} strideTAUP1 - stride length for `TAUP1` (in complex elements)
* @param {Complex128Array} TAUP2 - output array (length at least `M-P`)
...
* @throws {RangeError} `P` must satisfy `2*P >= M && P <= M`
* @throws {RangeError} `Q` must satisfy `Q >= M-P && M-Q >= M-P`
...
*/
function zunbdb3( order, M, P, Q, X11, LDX11, X21, LDX21, THETA, strideTHETA, PHI, stridePHI, TAUP1, strideTAUP1, TAUP2, strideTAUP2, TAUQ1, strideTAUQ1, WORK, strideWORK ) {
	var sx111;
	var sx112;
	var sx211;
	var sx212;
	...
	if ( 2 * P < M || P > M ) {
		throw new RangeError( format( 'invalid argument. Third argument must satisfy 2*P >= M and P <= M. Value: `%d`.', P ) );
	}
	if ( Q < M - P || M - Q < M - P ) {
		throw new RangeError( format( 'invalid argument. Fourth argument must satisfy Q >= M-P and M-Q >= M-P. Value: `%d`.', Q ) );
	}
