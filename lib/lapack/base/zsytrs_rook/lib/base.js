...
// Module-level scratch for cDiv result. Hot loops avoid allocation by reading these after each call.
var cdR = 0.0;
var cdI = 0.0;
...
/**
* Performs robust complex division `(ar+ai*i) / (br+bi*i)` using the Smith algorithm and stores the result in module-level `cdR` and `cdI`.
*
* @private
...
/**
* Solves a system of linear equations `A*X = B` with a complex symmetric matrix `A` using the factorization `A = U*D*U^T` or `A = L*D*L^T` computed by `zsytrf_rook`.
*
* IPIV uses the same 0-based convention as `zsytf2_rook`/`zsytrf_rook`:
*
* -   `IPIV[k]` >= 0: 1x1 pivot, row k was interchanged with row `IPIV[k]`.
* -   `IPIV[k]` < 0: 2x2 pivot, `IPIV[k]` = `~kp` (bitwise NOT of 0-based index). For rook pivoting, BOTH entries of a 2x2 block are negative, each encoding its own swap target.
*
* @private
...
* @param {NonNegativeInteger} offsetA - offset into `A` (in complex elements)
* @param {Int32Array} IPIV - pivot indices from `zsytrf_rook`
* @param {integer} strideIPIV - stride of `IPIV`
* @param {NonNegativeInteger} offsetIPIV - offset into `IPIV`
...
*/
function zsytrsRook( uplo, N, nrhs, A, strideA1, strideA2, offsetA, IPIV, strideIPIV, offsetIPIV, B, strideB1, strideB2, offsetB ) {
	var denomR;
	var denomI;
	var akm1kR;
	var akm1kI;
	...
	if ( uplo === 'upper' ) {
		...
		// Phase 1: Solve U*D*X' = B, overwriting B with X'. K starts at N-1 and decreases.
		k = N - 1;
		while ( k >= 0 ) {
			if ( IPIV[ offsetIPIV + (k * strideIPIV) ] >= 0 ) {
				...
				// AKM1 = A(k-1,k-1) / AKM1K  (symmetric: complex diagonal)
				p2 = (offsetA * 2) + (( k - 1 ) * sa1) + (( k - 1 ) * sa2);
				cDiv( Av[ p2 ], Av[ p2 + 1 ], akm1kR, akm1kI );
				akm1R = cdR;
				akm1I = cdI;

				// AK = A(k,k) / AKM1K  (symmetric: NO conjugation)
				...
				for ( j = 0; j < nrhs; j++ ) {
					...
					// BK = B(k,j) / AKM1K  (symmetric: NO conjugation)
		...
		// Phase 2: Solve U^T * X = X', computing X from X'. K starts at 0 and increases.
		k = 0;
		while ( k < N ) {
			if ( IPIV[ offsetIPIV + (k * strideIPIV) ] >= 0 ) {
				// 1x1 pivot block
		...
		// Phase 1: Solve L*D*X' = B, overwriting B with X'. K starts at 0 and increases.
		k = 0;
		while ( k < N ) {
			if ( IPIV[ offsetIPIV + (k * strideIPIV) ] >= 0 ) {
				// 1x1 pivot block
				...
				// Multiply by inv(D(K)): symmetric, complex diagonal
				p1 = (offsetA * 2) + (k * sa1) + (k * sa2);
				cDiv( 1.0, 0.0, Av[ p1 ], Av[ p1 + 1 ] );
				zscal( nrhs, new Complex128( cdR, cdI ), B, strideB2, offsetB + (k * strideB1) );
				k += 1;
				...
				// AKM1 = A(k,k) / AKM1K  (symmetric: NO conjugation)
				p2 = (offsetA * 2) + (k * sa1) + (k * sa2);
				cDiv( Av[ p2 ], Av[ p2 + 1 ], akm1kR, akm1kI );
				akm1R = cdR;
				akm1I = cdI;
				...
				for ( j = 0; j < nrhs; j++ ) {
					// BKM1 = B(k,j) / AKM1K  (symmetric: NO conjugation)
		...
		// Phase 2: Solve L^T * X = X', computing X from X'. K starts at N-1 and decreases.
		k = N - 1;
		while ( k >= 0 ) {
			if ( IPIV[ offsetIPIV + (k * strideIPIV) ] >= 0 ) {
				// 1x1 pivot block
