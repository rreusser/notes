...
interface Routine {
	...
	*/
	...
	/**
	* Computes a blocked Tall-Skinny LQ (TSLQ) factorization of a real `M`-by-`N` matrix (with `M <= N`), using alternative indexing semantics.
	*
	* @param M - number of rows of `A`
	* @param N - number of columns of `A`
	* @param mb - inner block size
