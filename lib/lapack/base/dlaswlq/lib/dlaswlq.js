...
* @param {PositiveInteger} mb - inner block size (`1 <= mb <= M` when `M > 0`)
...
* @throws {RangeError} third argument must be a nonnegative integer with `N >= M`
* @throws {RangeError} fourth argument must be a positive integer not exceeding `M` (when `M > 0`)
* @throws {RangeError} fifth argument must be a nonnegative integer
* @throws {RangeError} `LDA` must be valid for the chosen order
* @throws {RangeError} `LDT` must be greater than or equal to `mb`
* @returns {integer} status code (`0` = success)
*/
function dlaswlq( order, M, N, mb, nb, A, LDA, T, LDT, WORK ) {
	var sa1;
	var sa2;
	var st1;
	var st2;
	...
	if ( N < 0 || N < M ) {
		throw new RangeError( format( 'invalid argument. Third argument must be a nonnegative integer satisfying `N >= M`. Value: `%d`.', N ) );
	}
	if ( mb < 1 || ( mb > M && M > 0 ) ) {
		throw new RangeError( format( 'invalid argument. Fourth argument must be a positive integer not exceeding `M` (when `M > 0`). Value: `%d`.', mb ) );
	}
	if ( nb < 0 ) {
		throw new RangeError( format( 'invalid argument. Fifth argument must be a nonnegative integer. Value: `%d`.', nb ) );
	}
	if ( order === 'column-major' ) {
		...
		if ( LDT < max( 1, mb ) ) {
			throw new RangeError( format( 'invalid argument. Ninth argument must be greater than or equal to `mb`. Value: `%d`.', LDT ) );
		}
		sa1 = 1;
		sa2 = LDA;
		st1 = 1;
