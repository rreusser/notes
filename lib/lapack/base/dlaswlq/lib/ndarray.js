...
* @throws {RangeError} second argument must be a nonnegative integer with `N >= M`
* @throws {RangeError} third argument must be a positive integer not exceeding `M` (when `M > 0`)
...
function dlaswlq( M, N, mb, nb, A, strideA1, strideA2, offsetA, T, strideT1, strideT2, offsetT, WORK, strideWORK, offsetWORK ) {
	...
	if ( N < 0 || N < M ) {
		throw new RangeError( format( 'invalid argument. Second argument must be a nonnegative integer satisfying `N >= M`. Value: `%d`.', N ) );
	}
	if ( mb < 1 || ( mb > M && M > 0 ) ) {
		throw new RangeError( format( 'invalid argument. Third argument must be a positive integer not exceeding `M` (when `M > 0`). Value: `%d`.', mb ) );
	}
	if ( nb < 0 ) {
