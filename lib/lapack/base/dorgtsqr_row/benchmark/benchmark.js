...
function createBenchmark( M, N, MB, NB ) {
	...
	// Build a column-major matrix and factor with dlatsqr to produce input V/T.
	...
	*/
	function benchmark( b ) {
		...
		// Pre-allocate per-iteration copies.
		...
		for ( i = 0; i < b.iterations; i++ ) {
			...
			info = dorgtsqr_row( 'column-major', M, N, MB, NB, A, M, T, NB, WORK, 1 );
			if ( info !== 0 ) {
...
function main() {
	...
	for ( i = min; i <= max; i++ ) {
		...
		bench( format( '%s:M=%d,N=%d,MB=%d,NB=%d', pkg, M, N, 2 * N, N >> 1 ), f );
	}
}
