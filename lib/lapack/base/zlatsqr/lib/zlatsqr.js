...
*/
function zlatsqr( order, M, N, mb, nb, A, LDA, T, LDT, WORK ) {
	var sa1;
	var sa2;
	var st1;
	var st2;
	...
	if ( order === 'column-major' ) {
		...
		// Row-major T has shape (nb, N * Number_of_row_blocks). When mb<=N or mb>=M (single zgeqrt path), it has N columns; else it has N*ceil((M-N)/(mb-N)) columns. In row-major, LDT is the stride between rows.
		if ( LDT < nb ) {
			throw new RangeError( format( 'invalid argument. Ninth argument must be greater than or equal to `nb`. Value: `%d`.', LDT ) );
		}
		sa1 = LDA;
