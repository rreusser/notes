...
var zgeqrt = require( './../../zgeqrt/lib/base.js' );
var ztpqrt = require( './../../ztpqrt/lib/base.js' );
...
* ## Notes
*
* -   The factorization computes `A = Q * (R; 0)`, where `Q` is `M`-by-`M` unitary and `R` is `N`-by-`N` upper triangular.
...
* -   The first `mb`-by-`N` row block is factored via `zgeqrt`; subsequent row blocks are combined with the running `R` via `ztpqrt`.
* -   `T` is treated as `nb`-by-`(N * Number_of_row_blocks)`, where `Number_of_row_blocks = ceil((M-N) / (mb-N))`. When `mb <= N` or `mb >= M`, the routine simply calls `zgeqrt` and `T` is `nb`-by-`N`.
...
function zlatsqr( M, N, mb, nb, A, strideA1, strideA2, offsetA, T, strideT1, strideT2, offsetT, WORK, strideWORK, offsetWORK ) {
	...
	// If the row block size does not produce a non-trivial TSQR partition (mb <= N or mb >= M), defer to a single zgeqrt.
	if ( mb <= N || mb >= M ) {
		return zgeqrt( M, N, nb, A, strideA1, strideA2, offsetA, T, strideT1, strideT2, offsetT, WORK, strideWORK, offsetWORK );
	}
	...
	zgeqrt( mb, N, nb, A, strideA1, strideA2, offsetA, T, strideT1, strideT2, offsetT, WORK, strideWORK, offsetWORK );
	...
	for ( i = mb; i <= ii - ( mb - N ); i += mb - N ) {
		...
		// Ztpqrt with l=0: B is purely rectangular (mb-N) x N rows beneath A's triangular block.
		ztpqrt( mb - N, N, 0, nb, A, strideA1, strideA2, offsetA, A, strideA1, strideA2, offsetA + ( i * strideA1 ), T, strideT1, strideT2, offsetT + ( ctr * N * strideT2 ), WORK, strideWORK, offsetWORK );
		ctr += 1;
	}

	// Factor the trailing block A(ii:M-1, 0:N-1) of KK rows, if any.
	if ( ii < M ) {
		ztpqrt( kk, N, 0, nb, A, strideA1, strideA2, offsetA, A, strideA1, strideA2, offsetA + ( ii * strideA1 ), T, strideT1, strideT2, offsetT + ( ctr * N * strideT2 ), WORK, strideWORK, offsetWORK );
	}
	return 0;
}
