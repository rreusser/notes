...
var dgelqt3 = require( './../lib/ndarray.js' );


// FIXTURES //

var fixtureDir = path.join( __dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
var raw = readFileSync( path.join( fixtureDir, 'dgelqt3.jsonl' ), 'utf8' ); // eslint-disable-line node/no-sync
...
/**
* Runs dgelqt3 on the given input and verifies the output matches the fixture.
*
* @private
...
function runCase( caseName, M, N, rows, tol ) {
	...
	A = colmajor( rows, M, N );
	T = new Float64Array( M * M );
	info = dgelqt3( M, N, A, 1, M, 0, T, 1, M, 0 );
...
test( 'dgelqt3: m=1, n=4 (base case)', function t() {
	runCase( 'm1_n4', 1, 4, [ [ 2.0, 1.5, 0.5, -1.25 ] ], 1e-13 );
});

test( 'dgelqt3: m=1, n=1 (degenerate base case)', function t() {
	runCase( 'm1_n1', 1, 1, [ [ 3.5 ] ], 1e-13 );
});

test( 'dgelqt3: m=2, n=4 (single recursive split)', function t() {
	runCase( 'm2_n4', 2, 4, [
		[ 2.0, 1.5, 0.5, -1.0 ],
		[ 0.7, 3.0, 1.1, 0.4 ]
	], 1e-13 );
...
test( 'dgelqt3: m=3, n=5 (recursive M1=1, M2=2)', function t() {
	runCase( 'm3_n5', 3, 5, [
...
test( 'dgelqt3: m=4, n=6 (deeper recursion, even split)', function t() {
	runCase( 'm4_n6', 4, 6, [
...
test( 'dgelqt3: m=5, n=7 (uneven split)', function t() {
	runCase( 'm5_n7', 5, 7, [
...
test( 'dgelqt3: m=4, n=4 (square)', function t() {
	runCase( 'm4_n4', 4, 4, [
		[ 4.0, 0.5, 0.3, 0.2 ],
		[ 0.6, 3.5, 0.4, 0.1 ],
		[ 0.2, 0.4, 4.5, 0.7 ],
...
test( 'dgelqt3: m=0 quick return', function t() {
	var info;
	var A;
	var T;
	A = new Float64Array( 0 );
	T = new Float64Array( 0 );
	info = dgelqt3( 0, 0, A, 1, 1, 0, T, 1, 1, 0 );
	assert.strictEqual( info, 0 );
});

test( 'dgelqt3: works with non-trivial offsets and strides', function t() {
	var offsetA;
	var offsetT;
	var info;
	var rows;
	...
	offsetA = 7;
	offsetT = 3;
	A = new Float64Array( ( LDA * 4 ) + offsetA );
	T = new Float64Array( ( LDT * 2 ) + offsetT );
	rows = [
		[ 2.0, 1.5, 0.5, -1.0 ],
		[ 0.7, 3.0, 1.1, 0.4 ]
	];
	...
	info = dgelqt3( 2, 4, A, 1, LDA, offsetA, T, 1, LDT, offsetT );
	assert.strictEqual( info, 0 );

	// Pull out the 2x4 result region:
	Aout = new Float64Array( 8 );
	for ( j = 0; j < 4; j++ ) {
		for ( i = 0; i < 2; i++ ) {
			Aout[ ( j * 2 ) + i ] = A[ offsetA + ( j * LDA ) + i ];
		}
	}
	Tout = new Float64Array( 4 );
	for ( j = 0; j < 2; j++ ) {
		for ( i = 0; i < 2; i++ ) {
			Tout[ ( j * 2 ) + i ] = T[ offsetT + ( j * LDT ) + i ];
		}
	}
...
test( 'dgelqt3: works on a row-major matrix layout', function t() {
	var Aexp;
	var Texp;
	var rows;
	var info;
	...
	info = dgelqt3( M, N, A, N, 1, 0, T, M, 1, 0 );
	assert.strictEqual( info, 0 );
	...
	Texp = new Float64Array( M * M );
	for ( j = 0; j < M; j++ ) {
		for ( i = 0; i < M; i++ ) {
			Texp[ ( i * M ) + j ] = tc.T[ ( j * M ) + i ];
		}
	}
...
test( 'ndarray: throws when M is negative', function t() {
	assert.throws( function bad() {
		dgelqt3( -1, 3, new Float64Array( 9 ), 1, 3, 0, new Float64Array( 9 ), 1, 3, 0 ); // eslint-disable-line max-len
	}, RangeError );
});

test( 'ndarray: throws when N is negative', function t() {
	assert.throws( function bad() {
		dgelqt3( 1, -1, new Float64Array( 9 ), 1, 3, 0, new Float64Array( 9 ), 1, 3, 0 ); // eslint-disable-line max-len
	}, RangeError );
});

test( 'ndarray: throws when N < M', function t() {
	assert.throws( function bad() {
		dgelqt3( 4, 2, new Float64Array( 9 ), 1, 4, 0, new Float64Array( 16 ), 1, 4, 0 ); // eslint-disable-line max-len
	}, RangeError );
});
