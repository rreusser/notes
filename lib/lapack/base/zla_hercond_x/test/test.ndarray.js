...
var zla_hercond_x = require( './../lib/ndarray.js' );


// FIXTURES //

var fixtureDir = path.join( __dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
var lines = readFileSync( path.join( fixtureDir, 'zla_hercond_x.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
/**
* Converts a Fortran 1-based IPIV array into the 0-based JS convention (bitwise NOT for 2x2 pivot entries).
*
* @private
* @param {Array<number>} ipiv - Fortran pivot array
* @returns {Int32Array} JS pivot array
*/
function toIPIV( ipiv ) {
	var out;
	var i;
	var v;
...
/**
* Runs zla_hercond_x on a fixture case and checks the reciprocal condition.
*
* @private
...
function runCase( tc, uplo, N, tol, label ) {
	...
	result = zla_hercond_x( uplo, N, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, X, 1, 0, WORK, 1, 0, RWORK, 1, 0 );
...
test( 'zla_hercond_x: upper_4x4_real_x', function t() {
	var tc = findCase( 'upper_4x4_real_x' );
	runCase( tc, 'upper', 4, 1e-12, 'upper_4x4_real_x' );
});

test( 'zla_hercond_x: upper_4x4_complex_x', function t() {
	var merged;
	var base;
	var tc;
...
test( 'zla_hercond_x: lower_4x4_real_x', function t() {
	var tc = findCase( 'lower_4x4_real_x' );
	runCase( tc, 'lower', 4, 1e-12, 'lower_4x4_real_x' );
});

test( 'zla_hercond_x: lower_4x4_complex_x', function t() {
	var merged;
	var base;
	var tc;
...
test( 'zla_hercond_x: n1_upper', function t() {
	var tc = findCase( 'n1_upper' );
	runCase( tc, 'upper', 1, 1e-12, 'n1_upper' );
});

test( 'zla_hercond_x: n1_lower', function t() {
	var tc = findCase( 'n1_upper' );
	runCase( tc, 'lower', 1, 1e-12, 'n1_lower' );
});

test( 'zla_hercond_x: n0 returns 1.0', function t() {
	var result;
	var RWORK;
	var IPIV;
	var WORK;
	...
	result = zla_hercond_x( 'upper', 0, A, 1, 1, 0, AF, 1, 1, 0, IPIV, 1, 0, X, 1, 0, WORK, 1, 0, RWORK, 1, 0 );
	assert.equal( result, 1.0, 'n0 upper' );
	result = zla_hercond_x( 'lower', 0, A, 1, 1, 0, AF, 1, 1, 0, IPIV, 1, 0, X, 1, 0, WORK, 1, 0, RWORK, 1, 0 );
	assert.equal( result, 1.0, 'n0 lower' );
});

test( 'zla_hercond_x: anorm=0 returns 0 (zero matrix)', function t() {
	var result;
	var RWORK;
	var IPIV;
	var WORK;
	...
	WORK = new Complex128Array( 2 * N );
	RWORK = new Float64Array( N );
	result = zla_hercond_x( 'upper', N, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, X, 1, 0, WORK, 1, 0, RWORK, 1, 0 );
	assert.equal( result, 0.0, 'upper zero matrix' );
	result = zla_hercond_x( 'lower', N, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, X, 1, 0, WORK, 1, 0, RWORK, 1, 0 );
