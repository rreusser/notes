...
var zla_hercond_c = require( './../lib/ndarray.js' );


// FIXTURES //

var fixtureDir = path.join( __dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
var lines = readFileSync( path.join( fixtureDir, 'zla_hercond_c.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
/**
* Build the 3x3 Hermitian complex test matrix in column-major order.
*
* The matrix is Hermitian with real diagonal and conjugate mirror off-diagonals.
*
* @private
* @returns {Complex128Array} column-major 3x3 matrix
*/
function buildMatrix() {
	// Column-major: A(1,1), A(2,1), A(3,1), A(1,2), A(2,2), A(3,2), A(1,3), A(2,3), A(3,3)
	return new Complex128Array([
		// Column 1: A(1,1)=(2,0), A(2,1)=(1,-1), A(3,1)=(0,1)
		2.0,
		0.0,
		1.0,
		-1.0,
		...
		// Column 2: A(1,2)=(1,1), A(2,2)=(-3,0), A(3,2)=(2,-0.5)
		1.0,
		1.0,
		-3.0,
		0.0,
		...
		// Column 3: A(1,3)=(0,-1), A(2,3)=(2,0.5), A(3,3)=(4,0)
		0.0,
		-1.0,
		2.0,
		0.5,
...
function runCase( uplo, c, capply, fixtureName ) {
	...
	assert.equal( info, 0, fixtureName + ': zhetrf info' );
	res = zla_hercond_c( uplo, N, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, c, 1, 0, capply, WORK, 1, 0, RWORK, 1, 0 );
...
test( 'zla_hercond_c: n1_uplo_U', function t() {
	...
	AF = new Complex128Array( [ 5.0, 0.0 ] );
	IPIV = new Int32Array( [ 0 ] );
	c = new Float64Array( [ 2.0 ] );
	WORK = new Complex128Array( 2 * N );
	RWORK = new Float64Array( N );
	tc = findCase( 'n1_uplo_U' );
	res = zla_hercond_c( 'upper', N, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, c, 1, 0, true, WORK, 1, 0, RWORK, 1, 0 );
	assertClose( res, tc.result, 1e-12, 'n1_uplo_U' );
});

test( 'zla_hercond_c: n0', function t() {
	var RWORK;
	var IPIV;
	var WORK;
	var res;
	...
	res = zla_hercond_c( 'upper', 0, A, 1, 1, 0, AF, 1, 1, 0, IPIV, 1, 0, c, 1, 0, true, WORK, 1, 0, RWORK, 1, 0 );
	assert.equal( res, 1.0, 'n0' );
});

test( 'zla_hercond_c: anorm zero returns 0', function t() {
	var RWORK;
	var IPIV;
	var WORK;
	var res;
...
test( 'zla_hercond_c: lower anorm zero returns 0', function t() {
	var RWORK;
	var IPIV;
	var WORK;
	var res;
	...
	WORK = new Complex128Array( 2 * N );
	RWORK = new Float64Array( N );
	res = zla_hercond_c( 'lower', N, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, c, 1, 0, false, WORK, 1, 0, RWORK, 1, 0 );
