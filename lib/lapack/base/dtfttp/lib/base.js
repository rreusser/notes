...
*/
function dtfttp( transr, uplo, N, ARF, strideARF, offsetARF, AP, strideAP, offsetAP ) {
	var normalTransr;
	var nisodd;
	var lower;
	var ijp;
		...
		if ( normalTransr ) {
			...
			if ( lower ) {
				...
				for ( j = 0; j <= n2; j += 1 ) {
					for ( i = j; i <= N - 1; i += 1 ) {
						ij = i + jp;
						AP[ offsetAP + ( ijp * strideAP ) ] = ARF[ offsetARF + ( ij * strideARF ) ];
						ijp += 1;
					}
					jp += lda;
				}
