...
* @param {Complex128Array} WORK - workspace (length at least `M-Q`)
...
function zunbdb1( M, P, Q, X11, strideX111, strideX112, offsetX11, X21, strideX211, strideX212, offsetX21, THETA, strideTHETA, offsetTHETA, PHI, stridePHI, offsetPHI, TAUP1, strideTAUP1, offsetTAUP1, TAUP2, strideTAUP2, offsetTAUP2, TAUQ1, strideTAUQ1, offsetTAUQ1, WORK, strideWORK, offsetWORK ) {
	...
	if ( P < Q || M - P < Q ) {
		throw new RangeError( format( 'invalid argument. Second argument must satisfy P >= Q and M-P >= Q. Value: `%d`.', P ) );
	}
	if ( Q < 0 || M - Q < Q ) {
		throw new RangeError( format( 'invalid argument. Third argument must satisfy 0 <= Q <= M-Q. Value: `%d`.', Q ) );
	}
