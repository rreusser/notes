...
/**
* Solves a system of linear equations `A*X = B` with a complex Hermitian matrix `A` using the factorization `A = P*U*D*U^H*P^T` or `A = P*L*D*L^H*P^T` computed by `zhetrf_rk` (rook, bounded Bunch-Kaufman).
*
* The diagonal block matrix `D` has real diagonal entries (Hermitian) stored on the diagonal of `A`; complex super- (UPLO='upper') or sub- (UPLO='lower') diagonal entries that describe `2x2` pivot blocks live in `e`.
*
* IPIV uses the JS bitwise-NOT convention used by `zhetrf_rk`: positive 0-based indices for `1x1` pivot blocks, and `~kp` for `2x2` pivot blocks.
*
* @private
...
function zhetrs3( uplo, N, nrhs, A, strideA1, strideA2, offsetA, e, strideE, offsetE, IPIV, strideIPIV, offsetIPIV, B, strideB1, strideB2, offsetB ) {
	...
	if ( uplo === 'upper' ) {
		// Solve A*X = B, where A = P*U*D*U^H*P^T.
		...
		while ( i >= 0 ) {
			raw = IPIV[ offsetIPIV + ( i * strideIPIV ) ];
			if ( raw >= 0 ) {
				// 1x1 pivot block. Hermitian diagonal is real, so divide by the real part of A(i,i) using a real scalar.
				p1 = oaF + ( i * sa1 ) + ( i * sa2 );
				s = 1.0 / Av[ p1 ];
				...
				// AKM1 = A(i-1,i-1) / AKM1K   (Hermitian: A(i-1,i-1) is real)
				p2 = oaF + ( ( i - 1 ) * sa1 ) + ( ( i - 1 ) * sa2 );
				cDiv( Av[ p2 ], 0.0, akm1kR, akm1kI );
				akm1R = cdR;
				akm1I = cdI;

				// AK = A(i,i) / conj(AKM1K)   (Hermitian: A(i,i) is real)
				p2 = oaF + ( i * sa1 ) + ( i * sa2 );
				cDiv( Av[ p2 ], 0.0, akm1kR, -akm1kI );
				akR = cdR;
				akI = cdI;
		...
		// Solve A*X = B, where A = P*L*D*L^H*P^T.
		...
		while ( i < N ) {
			raw = IPIV[ offsetIPIV + ( i * strideIPIV ) ];
			if ( raw >= 0 ) {
				// 1x1 pivot block. Hermitian diagonal is real.
				p1 = oaF + ( i * sa1 ) + ( i * sa2 );
				s = 1.0 / Av[ p1 ];
				zdscal( nrhs, s, B, strideB2, offsetB + ( i * strideB1 ) );
			} else if ( i < N - 1 ) {
				...
				// AKM1 = A(i,i) / conj(AKM1K)   (Hermitian: A(i,i) is real)
				p2 = oaF + ( i * sa1 ) + ( i * sa2 );
				cDiv( Av[ p2 ], 0.0, akm1kR, -akm1kI );
				akm1R = cdR;
				akm1I = cdI;

				// AK = A(i+1,i+1) / AKM1K   (Hermitian: A(i+1,i+1) is real)
				p2 = oaF + ( ( i + 1 ) * sa1 ) + ( ( i + 1 ) * sa2 );
				cDiv( Av[ p2 ], 0.0, akm1kR, akm1kI );
				akR = cdR;
				akI = cdI;
