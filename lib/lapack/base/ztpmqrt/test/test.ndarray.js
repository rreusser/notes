...
test( 'ztpmqrt: WORK=null triggers internal allocation', function t() {
	var info;
	var vt;
	var tc;
	var A;
	...
	B = complexFromInterleaved( [ -0.5, 0.4, 0, 0.6, 0.5, 0.8, 1, 1, -1.5, 0.6, -1, 0.8, -0.5, 1, 0, 1.2, -2.5, 0.8, -2, 1, -1.5, 1.2, -1, 1.4 ] );
	vt = loadVT( 'factors_left_A' );
	info = ztpmqrt( 'left', 'no-transpose', 4, 3, 3, 2, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, A, 1, 3, 0, B, 1, 4, 0, null, 1, 0 );
	tc = findCase( 'left_notrans_A' );
	assert.equal( info, 0, 'info' );
	assertArrayClose( reinterpret( A, 0 ), tc.A, 1e-12, 'A' );
	assertArrayClose( reinterpret( B, 0 ), tc.B, 1e-12, 'B' );
...
test( 'ztpmqrt: undersized WORK triggers internal allocation (left side)', function t() {
...
test( 'ztpmqrt: undersized WORK triggers internal allocation (right side)', function t() {
	var WORK = new Complex128Array( 1 );
	var info;
	var vt;
	var tc;
