...
function createBenchmark( N ) {
	...
	// Fill A and B with non-zero values to avoid all-zero pathological paths.
	for ( i = 0; i < N * N; i++ ) {
		A.set( [ ( ( i % 7 ) - 3 ) * 0.1, ( ( i % 5 ) - 2 ) * 0.1 ], i );
		B.set( [ ( ( i % 11 ) - 5 ) * 0.05, ( ( i % 13 ) - 6 ) * 0.05 ], i );
	}
	return benchmark;
	...
	*/
	function benchmark( b ) {
		...
		for ( i = 0; i < b.iterations; i++ ) {
			y = ztpmqrt( 'column-major', 'left', 'no-transpose', N, N, N, l, nb, V, N, T, nb, A, N, B, N, WORK, 1 );
			if ( isnan( y ) ) {
