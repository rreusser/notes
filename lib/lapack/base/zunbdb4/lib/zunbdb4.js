...
* @param {Complex128Array} X11 - top block (`P`-by-`Q`)
* @param {PositiveInteger} LDX11 - leading dimension of `X11`
* @param {Complex128Array} X21 - bottom block (`(M-P)`-by-`Q`)
...
* @param {Complex128Array} TAUP1 - output array (length at least `M-Q`)
* @param {integer} strideTAUP1 - stride length for `TAUP1` (in complex elements)
* @param {Complex128Array} TAUP2 - output array (length at least `M-Q`)
* @param {integer} strideTAUP2 - stride length for `TAUP2` (in complex elements)
* @param {Complex128Array} TAUQ1 - output array (length at least `Q`)
* @param {integer} strideTAUQ1 - stride length for `TAUQ1` (in complex elements)
* @param {Complex128Array} PHANTOM - workspace and output (length at least `M`)
...
*/
function zunbdb4( order, M, P, Q, X11, LDX11, X21, LDX21, THETA, strideTHETA, PHI, stridePHI, TAUP1, strideTAUP1, TAUP2, strideTAUP2, TAUQ1, strideTAUQ1, PHANTOM, stridePHANTOM, WORK, strideWORK ) {
	var sx111;
	var sx112;
	var sx211;
	var sx212;
	...
	if ( P < M - Q || M - P < M - Q ) {
		throw new RangeError( format( 'invalid argument. Third argument must satisfy P >= M-Q and M-P >= M-Q. Value: `%d`.', P ) );
	}
	if ( Q < 0 || Q > M || M - Q > Q ) {
		throw new RangeError( format( 'invalid argument. Fourth argument must satisfy 0 <= Q <= M and M-Q <= Q. Value: `%d`.', Q ) );
	}
	...
	return base( M, P, Q, X11, sx111, sx112, 0, X21, sx211, sx212, 0, THETA, strideTHETA, 0, PHI, stridePHI, 0, TAUP1, strideTAUP1, 0, TAUP2, strideTAUP2, 0, TAUQ1, strideTAUQ1, 0, PHANTOM, stridePHANTOM, 0, WORK, strideWORK, 0 );
}
