...
* @param {(Float64Array|null)} WORK - workspace buffer, or `null` to allocate internally (LAPACKE-style convenience). Must contain at least `(max(1,N)*32 + 33*32)` elements (`side='left'`) or `(max(1,M)*32 + 33*32)` (`side='right'`).
...
*/
function dormqr( order, side, trans, M, N, K, A, LDA, TAU, strideTAU, C, LDC, WORK, strideWORK ) {
	var sa1;
	var sa2;
	var sc1;
	var sc2;
	...
	if ( WORK === null ) {
		...
		WORK = new Float64Array( ( nw * NB ) + ( ( NB + 1 ) * NB ) );
		strideWORK = 1;
		ow = 0;
	} else {
