...
var zsytri2 = require( './../lib/ndarray.js' );


// FIXTURES //

var fixtureDir = path.join( __dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' ); // eslint-disable-line max-len
var lines = readFileSync( path.join( fixtureDir, 'zsytri2.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line max-len
...
/**
* Converts a Fortran 1-based IPIV array into the JS convention used by `zsyconv`/`zsytri2x`: non-negative `0`-based indices for `1x1` blocks and bitwise-NOT-encoded indices for `2x2` blocks.
*
* @private
...
/**
* Asserts triangle equality against a reference column-major interleaved complex buffer with `LDA`-row storage.
*
* @private
...
/**
* Allocates a workspace sized for the worst case (matches the `zsytri2.js` BLAS-style wrapper).
*
* @private
...
function makeWork( N ) {
	var ldwork;
	if ( NBMAX >= N ) {
		return new Complex128Array( Math.max( 1, N ) );
	}
	ldwork = N + NBMAX + 1;
	return new Complex128Array( ldwork * ( NBMAX + 3 ) );
}
...
function runFixture( uplo, N, tc, tol ) {
	...
	work = makeWork( N );
	info = zsytri2( uplo, N, A, 1, LDA, 0, ipiv, 1, 0, work, 1, 0 );
	assert.equal( info, tc.info, 'info' );

	Aview = new Float64Array( A.buffer );
	assertTriangleClose( Aview, tc.a_inv, uplo, N, tol, 'a_inv' );
}


// TESTS //

test( 'zsytri2 ndarray is a function', function t() {
	assert.strictEqual( typeof zsytri2, 'function', 'is a function' );
});

test( 'zsytri2: 4x4 upper definite (unblocked path)', function t() {
	runFixture( 'upper', 4, findCase( '4x4_upper_def' ), 1e-10 );
});

test( 'zsytri2: 4x4 lower definite (unblocked path)', function t() {
	runFixture( 'lower', 4, findCase( '4x4_lower_def' ), 1e-10 );
});

test( 'zsytri2: 4x4 upper indefinite (2x2 pivots, unblocked path)', function t() {
	runFixture( 'upper', 4, findCase( '4x4_upper_indef' ), 1e-10 );
});

test( 'zsytri2: 4x4 lower indefinite (2x2 pivots, unblocked path)', function t() {
	runFixture( 'lower', 4, findCase( '4x4_lower_indef' ), 1e-10 );
});

test( 'zsytri2: 5x5 lower mixed pivots (unblocked path)', function t() {
	runFixture( 'lower', 5, findCase( '5x5_lower_mixed' ), 1e-10 );
});

test( 'zsytri2: 5x5 upper mixed pivots (unblocked path)', function t() {
	runFixture( 'upper', 5, findCase( '5x5_upper_mixed' ), 1e-10 );
});

test( 'zsytri2: N=1 trivial inverse (unblocked path)', function t() {
	var Aview;
	var ipiv;
	var work;
	var info;
	...
	ipiv = convertIpiv( tc.ipiv );
	work = makeWork( 1 );
	info = zsytri2( 'upper', 1, A, 1, LDA, 0, ipiv, 1, 0, work, 1, 0 );
	assert.equal( info, tc.info, 'info' );
	Aview = new Float64Array( A.buffer );
...
test( 'zsytri2: N=0 quick return', function t() {
	var ipiv;
	var work;
	var info;
	var A;
	...
	info = zsytri2( 'lower', 0, A, 1, 1, 0, ipiv, 1, 0, work, 1, 0 );
	assert.equal( info, 0, 'info' );
});

test( 'zsytri2: 40x40 lower well-conditioned (blocked path)', function t() {
	runFixture( 'lower', 40, findCase( '40x40_lower_dd' ), 1e-8 );
});

test( 'zsytri2: 40x40 upper well-conditioned (blocked path)', function t() {
	runFixture( 'upper', 40, findCase( '40x40_upper_dd' ), 1e-8 );
});

test( 'zsytri2: validation throws on invalid uplo', function t() {
	var ipiv;
	var work;
	var A;
	...
	assert.throws( function fn() {
		zsytri2( 'invalid', 2, A, 1, 2, 0, ipiv, 1, 0, work, 1, 0 );
	}, TypeError );
});

test( 'zsytri2: validation throws on negative N', function t() {
	var ipiv;
	var work;
	var A;
	A = new Complex128Array( 4 );
	...
	work = new Complex128Array( 10 );
	assert.throws( function fn() {
		zsytri2( 'upper', -1, A, 1, 2, 0, ipiv, 1, 0, work, 1, 0 );
	}, RangeError );
});
