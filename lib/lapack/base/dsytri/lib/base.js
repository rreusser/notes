...
* `A = U * D * U^T` or `A = L * D * L^T` computed by `dsytrf` or `dsytf2`.
*
* On entry, `A` contains the block diagonal matrix D and the multipliers
* used to obtain the factor U (or L) as computed by `dsytrf`, stored in
* full format. On exit, if `info === 0`, `A` contains the upper (or lower)
* triangle of the inverse of A.
*
* `IPIV` stores 0-based pivot indices from `dsytrf`/`dsytf2`. If `IPIV[k] >= 0`,
...
*/
function dsytri( uplo, N, A, strideA1, strideA2, offsetA, IPIV, strideIPIV, offsetIPIV, WORK, strideWORK, offsetWORK ) { // eslint-disable-line max-len
	var kstep;
	var akkp1;
	var temp;
	var info;
	...
	if ( uplo === 'upper' ) {
		...
		while ( true ) {
			...
			if ( kp !== k ) {
				// Interchange rows and columns k and kp in the leading submatrix A(1:k+1,1:k+1)
