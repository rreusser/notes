...
*
* @private
* @param {Float64Array} v - interleaved real/imag Float64 view
* @param {integer} idx - index into `v` (pointing to the real part)
...
* ## Notes
*
* -   The "max absolute element" norm is used (CABS1 norm for complex entries).
* -   If the result is much less than 1, the stability of the LU factorization of the (equilibrated) matrix A could be poor.
* -   `info` is the value returned from zsytrf (0 = success, k > 0 = zero pivot at column k, 1-based).
...
*/
function zla_syrpvgrw( uplo, N, info, A, strideA1, strideA2, offsetA, AF, strideAF1, strideAF2, offsetAF, IPIV, strideIPIV, offsetIPIV, WORK, strideWORK, offsetWORK ) {
	var rpvgrw;
	var ncols;
	var upper;
	var amax;
	...
	if ( upper ) {
		// NOTE: This while loop is unreachable in the upper branch. K starts at N
		...
		while ( (k + 1) < ncols && k >= 0 ) {
			...
			if ( kp >= 0 ) {
				...
				for ( i = 0; i < k; i += 1 ) {
					...
					tmp = cabs1( Fv, oF + (i * sf1) + ((k - 1) * sf2) );
					if ( tmp > WORK[ oW + ((k - 1) * sW) ] ) {
						WORK[ oW + ((k - 1) * sW) ] = tmp;
					}
				}
				tmp = cabs1( Fv, oF + (k * sf1) + (k * sf2) );
				if ( tmp > WORK[ oW + (k * sW) ] ) {
					WORK[ oW + (k * sW) ] = tmp;
				}
				k -= 2;
		...
		while ( k < ncols ) {
			...
			if ( kp >= 0 ) {
				...
				// NOTE: This diagonal update branch is hard to trigger because ZSYTRF
	...
	if ( upper ) {
		// NOTE: In the upper branch, the first-pass while loop (above) never executes
		// (see NOTE there), so WORK[0..N-1] remains 0. As a result, the umax !== 0
