...
FIXTURES.forEach( function each( fx ) {
	test( 'ndarray fixture: ' + fx.name, function t() {
		...
		rpvgrw = zla_syrpvgrw.ndarray( fx.name.indexOf( 'upper' ) >= 0 ? 'upper' : 'lower', N, info, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, WORK, 1, 0 );
...
test( 'ndarray N=0 returns 1', function t() {
	var rpvgrw = zla_syrpvgrw.ndarray( 'upper', 0, 0, new Complex128Array( 0 ), 1, 1, 0, new Complex128Array( 0 ), 1, 1, 0, new Int32Array( 0 ), 1, 0, new Float64Array( 0 ), 1, 0 );
...
test( 'ndarray triggers diagonal update branch (lower 2x2 pivot, AF[k,k] dominant)', function t() {
	...
	rpvgrw = zla_syrpvgrw.ndarray( 'lower', N, 0, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, WORK, 1, 0 );
...
test( 'ndarray exercises INFO > 0 (singular) path', function t() {
	...
	var WORK = new Float64Array( 2 * N );
	var r = zla_syrpvgrw.ndarray( 'lower', N, 2, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, WORK, 1, 0 );
...
test( 'ndarray rpvgrw < 1 when factor grows (lower)', function t() {
	...
	var rpvgrw = zla_syrpvgrw.ndarray( 'lower', N, 0, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, WORK, 1, 0 );
	approxEqual( rpvgrw, 0.1, 1e-12, 'rpvgrw' );
});
