...
function createBenchmark( N ) {
	...
	for ( j = 0; j < N; j++ ) {
		for ( i = j; i < N; i++ ) {
			idx = ( ( j * N ) + i ) * 2;
			Aref[ idx ] = Math.sin( ( i + 1 ) * 0.7 ) * Math.cos( ( j + 1 ) * 0.3 ); // eslint-disable-line max-len
			Aref[ idx + 1 ] = ( i === j ) ? 0.0 : 0.2 * Math.sin( ( i - j ) * 0.21 ); // eslint-disable-line max-len
		}
		idx = ( ( j * N ) + j ) * 2;
		Aref[ idx ] = N + 1.0;
		Aref[ idx + 1 ] = 0.0;
	...
	*/
	function benchmark( b ) {
		...
		for ( k = 0; k < b.iterations; k++ ) {
			...
			y = zhetrfRook( 'lower', N, A, 1, N, 0, IPIV, 1, 0 );
			if ( isnan( y ) ) {
