...
test( 'zdrot: negative N is a no-op', function t() {
	...
	zy = new Complex128Array( [ 3.0, 4.0 ] );
	result = base( -1, zx, 1, 0, zy, 1, 0, 0.6, 0.8 );
	assert.strictEqual( result, zx );
	assertArrayClose( toArray( reinterpret( zx, 0 ) ), [ 1.0, 2.0 ], 'zx unchanged' ); // eslint-disable-line max-len
	assertArrayClose( toArray( reinterpret( zy, 0 ) ), [ 3.0, 4.0 ], 'zy unchanged' ); // eslint-disable-line max-len
});
