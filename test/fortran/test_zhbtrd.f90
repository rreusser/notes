program test_zhbtrd
  use test_utils
  implicit none

  integer :: INFO, N, KD, LDAB, LDQ, I

  ! Complex arrays with EQUIVALENCE for printing
  complex*16 :: AB(100), Q(100), WORK(20)
  double precision :: AB_r(200), Q_r(200), WORK_r(40)
  equivalence (AB, AB_r)
  equivalence (Q, Q_r)
  equivalence (WORK, WORK_r)

  double precision :: D(10), E(10)

  ! =====================================================================
  ! Test 1: UPLO='U', KD=2, N=5, VECT='N' (no Q)
  !
  ! Hermitian 5x5 matrix:
  !   4      (1+i)   (2-i)   0       0
  !  (1-i)    5      (3+i)  (1-i)    0
  !  (2+i)   (3-i)    6     (2+i)   (1-i)
  !   0      (1+i)   (2-i)   7      (3+i)
  !   0       0      (1+i)  (3-i)    8
  !
  ! Upper band storage (LDAB=3=KD+1):
  !   Row 1 (2nd superdiag): *       *      (2-i)  (1-i)  (1-i)
  !   Row 2 (1st superdiag): *      (1+i)  (3+i)  (2+i)  (3+i)
  !   Row 3 (diagonal):      4       5      6      7      8
  ! =====================================================================
  N = 5; KD = 2; LDAB = 3; LDQ = 1
  AB = (0.0d0, 0.0d0); D = 0.0d0; E = 0.0d0; Q = (0.0d0, 0.0d0); WORK = (0.0d0, 0.0d0)

  ! Column 1: AB(3,1)=4
  AB(3) = (4.0d0, 0.0d0)
  ! Column 2: AB(2,2)=(1+i), AB(3,2)=5
  AB(5) = (1.0d0, 1.0d0)
  AB(6) = (5.0d0, 0.0d0)
  ! Column 3: AB(1,3)=(2-i), AB(2,3)=(3+i), AB(3,3)=6
  AB(7) = (2.0d0, -1.0d0)
  AB(8) = (3.0d0, 1.0d0)
  AB(9) = (6.0d0, 0.0d0)
  ! Column 4: AB(1,4)=(1-i), AB(2,4)=(2+i), AB(3,4)=7
  AB(10) = (1.0d0, -1.0d0)
  AB(11) = (2.0d0, 1.0d0)
  AB(12) = (7.0d0, 0.0d0)
  ! Column 5: AB(1,5)=(1-i), AB(2,5)=(3+i), AB(3,5)=8
  AB(13) = (1.0d0, -1.0d0)
  AB(14) = (3.0d0, 1.0d0)
  AB(15) = (8.0d0, 0.0d0)

  call ZHBTRD('N', 'U', N, KD, AB, LDAB, D, E, Q, LDQ, WORK, INFO)
  call begin_test('upper_kd2_n5_none')
  call print_int('info', INFO)
  call print_array('d', D, 5)
  call print_array('e', E, 4)
  call end_test()

  ! =====================================================================
  ! Test 2: Same matrix, UPLO='L', KD=2, N=5, VECT='N'
  !
  ! Lower band storage (LDAB=3=KD+1):
  !   Row 1 (diagonal):      4       5      6      7      8
  !   Row 2 (1st subdiag):  (1-i)   (3-i)  (2-i)  (3-i)   *
  !   Row 3 (2nd subdiag):  (2+i)   (1+i)  (1+i)   *      *
  ! =====================================================================
  N = 5; KD = 2; LDAB = 3; LDQ = 1
  AB = (0.0d0, 0.0d0); D = 0.0d0; E = 0.0d0; Q = (0.0d0, 0.0d0); WORK = (0.0d0, 0.0d0)

  ! Column 1: AB(1,1)=4, AB(2,1)=(1-i), AB(3,1)=(2+i)
  AB(1) = (4.0d0, 0.0d0)
  AB(2) = (1.0d0, -1.0d0)
  AB(3) = (2.0d0, 1.0d0)
  ! Column 2: AB(1,2)=5, AB(2,2)=(3-i), AB(3,2)=(1+i)
  AB(4) = (5.0d0, 0.0d0)
  AB(5) = (3.0d0, -1.0d0)
  AB(6) = (1.0d0, 1.0d0)
  ! Column 3: AB(1,3)=6, AB(2,3)=(2-i), AB(3,3)=(1+i)
  AB(7) = (6.0d0, 0.0d0)
  AB(8) = (2.0d0, -1.0d0)
  AB(9) = (1.0d0, 1.0d0)
  ! Column 4: AB(1,4)=7, AB(2,4)=(3-i)
  AB(10) = (7.0d0, 0.0d0)
  AB(11) = (3.0d0, -1.0d0)
  ! Column 5: AB(1,5)=8
  AB(13) = (8.0d0, 0.0d0)

  call ZHBTRD('N', 'L', N, KD, AB, LDAB, D, E, Q, LDQ, WORK, INFO)
  call begin_test('lower_kd2_n5_none')
  call print_int('info', INFO)
  call print_array('d', D, 5)
  call print_array('e', E, 4)
  call end_test()

  ! =====================================================================
  ! Test 3: UPLO='U', KD=1, N=4, VECT='N' (tridiagonal input)
  !
  ! Hermitian 4x4 tridiagonal:
  !   4     (1+i)   0      0
  !  (1-i)   5     (2-i)   0
  !   0     (2+i)   6     (3+i)
  !   0      0     (3-i)   7
  !
  ! Upper band (LDAB=2=KD+1):
  !   Row 1 (1st superdiag): *      (1+i)  (2-i)  (3+i)
  !   Row 2 (diagonal):      4       5      6      7
  ! =====================================================================
  N = 4; KD = 1; LDAB = 2; LDQ = 1
  AB = (0.0d0, 0.0d0); D = 0.0d0; E = 0.0d0; Q = (0.0d0, 0.0d0); WORK = (0.0d0, 0.0d0)

  ! Col 1: AB(2,1)=4
  AB(2) = (4.0d0, 0.0d0)
  ! Col 2: AB(1,2)=(1+i), AB(2,2)=5
  AB(3) = (1.0d0, 1.0d0)
  AB(4) = (5.0d0, 0.0d0)
  ! Col 3: AB(1,3)=(2-i), AB(2,3)=6
  AB(5) = (2.0d0, -1.0d0)
  AB(6) = (6.0d0, 0.0d0)
  ! Col 4: AB(1,4)=(3+i), AB(2,4)=7
  AB(7) = (3.0d0, 1.0d0)
  AB(8) = (7.0d0, 0.0d0)

  call ZHBTRD('N', 'U', N, KD, AB, LDAB, D, E, Q, LDQ, WORK, INFO)
  call begin_test('upper_kd1_n4_none')
  call print_int('info', INFO)
  call print_array('d', D, 4)
  call print_array('e', E, 3)
  call end_test()

  ! =====================================================================
  ! Test 4: UPLO='L', KD=1, N=4, VECT='N'
  !
  ! Lower band (LDAB=2=KD+1):
  !   Row 1 (diagonal):      4       5      6      7
  !   Row 2 (1st subdiag):  (1-i)   (2+i)  (3-i)   *
  ! =====================================================================
  N = 4; KD = 1; LDAB = 2; LDQ = 1
  AB = (0.0d0, 0.0d0); D = 0.0d0; E = 0.0d0; Q = (0.0d0, 0.0d0); WORK = (0.0d0, 0.0d0)

  AB(1) = (4.0d0, 0.0d0); AB(2) = (1.0d0, -1.0d0)
  AB(3) = (5.0d0, 0.0d0); AB(4) = (2.0d0, 1.0d0)
  AB(5) = (6.0d0, 0.0d0); AB(6) = (3.0d0, -1.0d0)
  AB(7) = (7.0d0, 0.0d0)

  call ZHBTRD('N', 'L', N, KD, AB, LDAB, D, E, Q, LDQ, WORK, INFO)
  call begin_test('lower_kd1_n4_none')
  call print_int('info', INFO)
  call print_array('d', D, 4)
  call print_array('e', E, 3)
  call end_test()

  ! =====================================================================
  ! Test 5: UPLO='U', KD=2, N=5, VECT='V' (initialize Q)
  ! Same matrix as test 1
  ! =====================================================================
  N = 5; KD = 2; LDAB = 3; LDQ = 5
  AB = (0.0d0, 0.0d0); D = 0.0d0; E = 0.0d0; Q = (0.0d0, 0.0d0); WORK = (0.0d0, 0.0d0)

  AB(3) = (4.0d0, 0.0d0)
  AB(5) = (1.0d0, 1.0d0); AB(6) = (5.0d0, 0.0d0)
  AB(7) = (2.0d0, -1.0d0); AB(8) = (3.0d0, 1.0d0); AB(9) = (6.0d0, 0.0d0)
  AB(10) = (1.0d0, -1.0d0); AB(11) = (2.0d0, 1.0d0); AB(12) = (7.0d0, 0.0d0)
  AB(13) = (1.0d0, -1.0d0); AB(14) = (3.0d0, 1.0d0); AB(15) = (8.0d0, 0.0d0)

  call ZHBTRD('V', 'U', N, KD, AB, LDAB, D, E, Q, LDQ, WORK, INFO)
  call begin_test('upper_kd2_n5_init')
  call print_int('info', INFO)
  call print_array('d', D, 5)
  call print_array('e', E, 4)
  call print_array('Q', Q_r, 2*N*N)
  call end_test()

  ! =====================================================================
  ! Test 6: UPLO='L', KD=2, N=5, VECT='V' (initialize Q)
  ! Same matrix as test 2
  ! =====================================================================
  N = 5; KD = 2; LDAB = 3; LDQ = 5
  AB = (0.0d0, 0.0d0); D = 0.0d0; E = 0.0d0; Q = (0.0d0, 0.0d0); WORK = (0.0d0, 0.0d0)

  AB(1) = (4.0d0, 0.0d0); AB(2) = (1.0d0, -1.0d0); AB(3) = (2.0d0, 1.0d0)
  AB(4) = (5.0d0, 0.0d0); AB(5) = (3.0d0, -1.0d0); AB(6) = (1.0d0, 1.0d0)
  AB(7) = (6.0d0, 0.0d0); AB(8) = (2.0d0, -1.0d0); AB(9) = (1.0d0, 1.0d0)
  AB(10) = (7.0d0, 0.0d0); AB(11) = (3.0d0, -1.0d0)
  AB(13) = (8.0d0, 0.0d0)

  call ZHBTRD('V', 'L', N, KD, AB, LDAB, D, E, Q, LDQ, WORK, INFO)
  call begin_test('lower_kd2_n5_init')
  call print_int('info', INFO)
  call print_array('d', D, 5)
  call print_array('e', E, 4)
  call print_array('Q', Q_r, 2*N*N)
  call end_test()

  ! =====================================================================
  ! Test 7: N=0, quick return
  ! =====================================================================
  N = 0; KD = 0; LDAB = 1; LDQ = 1
  AB = (0.0d0, 0.0d0); D = 0.0d0; E = 0.0d0; Q = (0.0d0, 0.0d0); WORK = (0.0d0, 0.0d0)
  call ZHBTRD('N', 'U', N, KD, AB, LDAB, D, E, Q, LDQ, WORK, INFO)
  call begin_test('n_zero')
  call print_int('info', INFO)
  call end_test()

  ! =====================================================================
  ! Test 8: N=1, UPLO='U', VECT='V'
  ! =====================================================================
  N = 1; KD = 0; LDAB = 1; LDQ = 1
  AB = (0.0d0, 0.0d0); D = 0.0d0; E = 0.0d0; Q = (0.0d0, 0.0d0); WORK = (0.0d0, 0.0d0)
  AB(1) = (7.0d0, 0.0d0)
  call ZHBTRD('V', 'U', N, KD, AB, LDAB, D, E, Q, LDQ, WORK, INFO)
  call begin_test('n_one_upper_init')
  call print_int('info', INFO)
  call print_array('d', D, 1)
  call print_array('Q', Q_r, 2)
  call end_test()

  ! =====================================================================
  ! Test 9: KD=0 (diagonal matrix), UPLO='U', VECT='N'
  ! A = diag(2, 5, 8)
  ! =====================================================================
  N = 3; KD = 0; LDAB = 1; LDQ = 1
  AB = (0.0d0, 0.0d0); D = 0.0d0; E = 0.0d0; Q = (0.0d0, 0.0d0); WORK = (0.0d0, 0.0d0)
  AB(1) = (2.0d0, 0.0d0); AB(2) = (5.0d0, 0.0d0); AB(3) = (8.0d0, 0.0d0)

  call ZHBTRD('N', 'U', N, KD, AB, LDAB, D, E, Q, LDQ, WORK, INFO)
  call begin_test('kd0_upper_none')
  call print_int('info', INFO)
  call print_array('d', D, 3)
  call print_array('e', E, 2)
  call end_test()

  ! =====================================================================
  ! Test 10: UPLO='U', KD=3, N=6, VECT='V' (larger bandwidth)
  !
  ! Hermitian 6x6 with bandwidth 3:
  !   5      (2+i)   (1-i)  (3+i)   0       0
  !  (2-i)    6      (3-i)  (1+i)  (2-i)    0
  !  (1+i)   (3+i)    7    (2-i)   (3+i)   (1-i)
  !  (3-i)   (1-i)   (2+i)  8     (1+i)   (2+i)
  !   0      (2+i)   (3-i) (1-i)    9      (3-i)
  !   0       0      (1+i) (2-i)   (3+i)   10
  !
  ! Upper band (LDAB=4=KD+1):
  !   Row 1 (3rd superdiag): *       *       *     (3+i)  (2-i)  (1-i)
  !   Row 2 (2nd superdiag): *       *      (1-i)  (1+i)  (3+i)  (2+i)
  !   Row 3 (1st superdiag): *      (2+i)  (3-i)  (2-i)  (1+i)  (3-i)
  !   Row 4 (diagonal):      5       6      7      8      9      10
  ! =====================================================================
  N = 6; KD = 3; LDAB = 4; LDQ = 6
  AB = (0.0d0, 0.0d0); D = 0.0d0; E = 0.0d0; Q = (0.0d0, 0.0d0); WORK = (0.0d0, 0.0d0)

  ! Col 1: AB(4,1)=5
  AB(4) = (5.0d0, 0.0d0)
  ! Col 2: AB(3,2)=(2+i), AB(4,2)=6
  AB(7) = (2.0d0, 1.0d0)
  AB(8) = (6.0d0, 0.0d0)
  ! Col 3: AB(2,3)=(1-i), AB(3,3)=(3-i), AB(4,3)=7
  AB(10) = (1.0d0, -1.0d0)
  AB(11) = (3.0d0, -1.0d0)
  AB(12) = (7.0d0, 0.0d0)
  ! Col 4: AB(1,4)=(3+i), AB(2,4)=(1+i), AB(3,4)=(2-i), AB(4,4)=8
  AB(13) = (3.0d0, 1.0d0)
  AB(14) = (1.0d0, 1.0d0)
  AB(15) = (2.0d0, -1.0d0)
  AB(16) = (8.0d0, 0.0d0)
  ! Col 5: AB(1,5)=(2-i), AB(2,5)=(3+i), AB(3,5)=(1+i), AB(4,5)=9
  AB(17) = (2.0d0, -1.0d0)
  AB(18) = (3.0d0, 1.0d0)
  AB(19) = (1.0d0, 1.0d0)
  AB(20) = (9.0d0, 0.0d0)
  ! Col 6: AB(1,6)=(1-i), AB(2,6)=(2+i), AB(3,6)=(3-i), AB(4,6)=10
  AB(21) = (1.0d0, -1.0d0)
  AB(22) = (2.0d0, 1.0d0)
  AB(23) = (3.0d0, -1.0d0)
  AB(24) = (10.0d0, 0.0d0)

  call ZHBTRD('V', 'U', N, KD, AB, LDAB, D, E, Q, LDQ, WORK, INFO)
  call begin_test('upper_kd3_n6_init')
  call print_int('info', INFO)
  call print_array('d', D, 6)
  call print_array('e', E, 5)
  call print_array('Q', Q_r, 2*N*N)
  call end_test()

  ! =====================================================================
  ! Test 11: UPLO='L', KD=3, N=6, VECT='V', same matrix
  !
  ! Lower band (LDAB=4=KD+1):
  !   Row 1 (diagonal):      5       6      7      8      9      10
  !   Row 2 (1st subdiag):  (2-i)   (3+i)  (2+i)  (1-i)  (3+i)   *
  !   Row 3 (2nd subdiag):  (1+i)   (1-i)  (3-i)  (2-i)   *      *
  !   Row 4 (3rd subdiag):  (3-i)   (2+i)  (1+i)   *      *      *
  ! =====================================================================
  N = 6; KD = 3; LDAB = 4; LDQ = 6
  AB = (0.0d0, 0.0d0); D = 0.0d0; E = 0.0d0; Q = (0.0d0, 0.0d0); WORK = (0.0d0, 0.0d0)

  ! Col 1: diag=5, sub1=(2-i), sub2=(1+i), sub3=(3-i)
  AB(1) = (5.0d0, 0.0d0); AB(2) = (2.0d0, -1.0d0); AB(3) = (1.0d0, 1.0d0); AB(4) = (3.0d0, -1.0d0)
  ! Col 2: diag=6, sub1=(3+i), sub2=(1-i), sub3=(2+i)
  AB(5) = (6.0d0, 0.0d0); AB(6) = (3.0d0, 1.0d0); AB(7) = (1.0d0, -1.0d0); AB(8) = (2.0d0, 1.0d0)
  ! Col 3: diag=7, sub1=(2+i), sub2=(3-i), sub3=(1+i)
  AB(9) = (7.0d0, 0.0d0); AB(10) = (2.0d0, 1.0d0); AB(11) = (3.0d0, -1.0d0); AB(12) = (1.0d0, 1.0d0)
  ! Col 4: diag=8, sub1=(1-i), sub2=(2-i)
  AB(13) = (8.0d0, 0.0d0); AB(14) = (1.0d0, -1.0d0); AB(15) = (2.0d0, -1.0d0)
  ! Col 5: diag=9, sub1=(3+i)
  AB(17) = (9.0d0, 0.0d0); AB(18) = (3.0d0, 1.0d0)
  ! Col 6: diag=10
  AB(21) = (10.0d0, 0.0d0)

  call ZHBTRD('V', 'L', N, KD, AB, LDAB, D, E, Q, LDQ, WORK, INFO)
  call begin_test('lower_kd3_n6_init')
  call print_int('info', INFO)
  call print_array('d', D, 6)
  call print_array('e', E, 5)
  call print_array('Q', Q_r, 2*N*N)
  call end_test()

  ! =====================================================================
  ! Test 12: UPLO='U', KD=1, N=4, VECT='V' (tridiagonal, initialize Q)
  ! Same matrix as test 3
  ! =====================================================================
  N = 4; KD = 1; LDAB = 2; LDQ = 4
  AB = (0.0d0, 0.0d0); D = 0.0d0; E = 0.0d0; Q = (0.0d0, 0.0d0); WORK = (0.0d0, 0.0d0)

  AB(2) = (4.0d0, 0.0d0)
  AB(3) = (1.0d0, 1.0d0); AB(4) = (5.0d0, 0.0d0)
  AB(5) = (2.0d0, -1.0d0); AB(6) = (6.0d0, 0.0d0)
  AB(7) = (3.0d0, 1.0d0); AB(8) = (7.0d0, 0.0d0)

  call ZHBTRD('V', 'U', N, KD, AB, LDAB, D, E, Q, LDQ, WORK, INFO)
  call begin_test('upper_kd1_n4_init')
  call print_int('info', INFO)
  call print_array('d', D, 4)
  call print_array('e', E, 3)
  call print_array('Q', Q_r, 2*N*N)
  call end_test()

  ! =====================================================================
  ! Test 13: UPLO='L', KD=1, N=4, VECT='V' (tridiagonal, initialize Q)
  ! Same matrix as test 4
  ! =====================================================================
  N = 4; KD = 1; LDAB = 2; LDQ = 4
  AB = (0.0d0, 0.0d0); D = 0.0d0; E = 0.0d0; Q = (0.0d0, 0.0d0); WORK = (0.0d0, 0.0d0)

  AB(1) = (4.0d0, 0.0d0); AB(2) = (1.0d0, -1.0d0)
  AB(3) = (5.0d0, 0.0d0); AB(4) = (2.0d0, 1.0d0)
  AB(5) = (6.0d0, 0.0d0); AB(6) = (3.0d0, -1.0d0)
  AB(7) = (7.0d0, 0.0d0)

  call ZHBTRD('V', 'L', N, KD, AB, LDAB, D, E, Q, LDQ, WORK, INFO)
  call begin_test('lower_kd1_n4_init')
  call print_int('info', INFO)
  call print_array('d', D, 4)
  call print_array('e', E, 3)
  call print_array('Q', Q_r, 2*N*N)
  call end_test()

end program
