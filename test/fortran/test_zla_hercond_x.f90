program test_zla_hercond_x
  ...
  double precision :: ZLA_HERCOND_X
  external :: ZLA_HERCOND_X, ZHETRF

  LWORK = NMAX * 32

  ! ===== Test 1: UPLO='U', 4x4 well-conditioned Hermitian, real X =====
  n = 4
  A = (0.0d0, 0.0d0)
  A(1,1) = (4.0d0, 0.0d0)
  A(1,2) = (1.0d0, 2.0d0)
  ...
  tmp = ZLA_HERCOND_X('U', n, A, NMAX, AF, NMAX, IPIV, X, INFO, WORK, RWORK)
  do j = 1, n
    do i = 1, n
      AFpk(i + (j-1)*n) = AF(i, j)
      Apk(i + (j-1)*n) = A(i, j)
  ...
  ! ===== Test 3: UPLO='L', 4x4 Hermitian (lower-stored), real X =====
  n = 4
  A = (0.0d0, 0.0d0)
  A(1,1) = (4.0d0, 0.0d0)
  A(2,1) = (1.0d0, -2.0d0)
  ...
  X(1) = (1.0d0, 0.0d0); X(2) = (1.0d0, 0.0d0)
  X(3) = (1.0d0, 0.0d0); X(4) = (1.0d0, 0.0d0)
  tmp = ZLA_HERCOND_X('L', n, A, NMAX, AF, NMAX, IPIV, X, INFO, WORK, RWORK)
  do j = 1, n
    do i = 1, n
      AFpk(i + (j-1)*n) = AF(i, j)
      Apk(i + (j-1)*n) = A(i, j)
