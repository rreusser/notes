program test_zpotrs
  use test_utils
  implicit none
  complex*16 :: a(100), b(100)
  double precision :: a_r(200), b_r(200)
  equivalence (a, a_r)
  equivalence (b, b_r)
  integer :: info

  ! Hermitian positive definite 3x3 matrix (col-major):
  ! A = [10  3-i  1+2i;  3+i  8  2-i;  1-2i  2+i  6]

  ! Test 1: lower Cholesky solve, single RHS
  a = (0.0d0, 0.0d0); b = (0.0d0, 0.0d0)
  a(1) = (10.0d0, 0.0d0); a(2) = (3.0d0, 1.0d0); a(3) = (1.0d0, -2.0d0)
  a(4) = (3.0d0, -1.0d0); a(5) = (8.0d0, 0.0d0); a(6) = (2.0d0, 1.0d0)
  a(7) = (1.0d0, 2.0d0); a(8) = (2.0d0, -1.0d0); a(9) = (6.0d0, 0.0d0)
  b(1) = (1.0d0, 1.0d0); b(2) = (2.0d0, -1.0d0); b(3) = (3.0d0, 0.5d0)
  call zpotrf('L', 3, a, 3, info)
  call zpotrs('L', 3, 1, a, 3, b, 3, info)
  call begin_test('lower_single_rhs')
  call print_array('x', b_r, 6)
  call print_int('info', info)
  call end_test()

  ! Test 2: upper Cholesky solve, single RHS
  a = (0.0d0, 0.0d0); b = (0.0d0, 0.0d0)
  a(1) = (10.0d0, 0.0d0); a(2) = (3.0d0, 1.0d0); a(3) = (1.0d0, -2.0d0)
  a(4) = (3.0d0, -1.0d0); a(5) = (8.0d0, 0.0d0); a(6) = (2.0d0, 1.0d0)
  a(7) = (1.0d0, 2.0d0); a(8) = (2.0d0, -1.0d0); a(9) = (6.0d0, 0.0d0)
  b(1) = (1.0d0, 1.0d0); b(2) = (2.0d0, -1.0d0); b(3) = (3.0d0, 0.5d0)
  call zpotrf('U', 3, a, 3, info)
  call zpotrs('U', 3, 1, a, 3, b, 3, info)
  call begin_test('upper_single_rhs')
  call print_array('x', b_r, 6)
  call print_int('info', info)
  call end_test()

  ! Test 3: lower, multiple RHS (NRHS=2)
  a = (0.0d0, 0.0d0); b = (0.0d0, 0.0d0)
  a(1) = (10.0d0, 0.0d0); a(2) = (3.0d0, 1.0d0); a(3) = (1.0d0, -2.0d0)
  a(4) = (3.0d0, -1.0d0); a(5) = (8.0d0, 0.0d0); a(6) = (2.0d0, 1.0d0)
  a(7) = (1.0d0, 2.0d0); a(8) = (2.0d0, -1.0d0); a(9) = (6.0d0, 0.0d0)
  ! B is 3x2 col-major: first column (1+0i, 0, 0), second column (0, 1+0i, 0)
  b(1) = (1.0d0, 0.0d0); b(2) = (0.0d0, 0.0d0); b(3) = (0.0d0, 0.0d0)
  b(4) = (0.0d0, 0.0d0); b(5) = (1.0d0, 0.0d0); b(6) = (0.0d0, 0.0d0)
  call zpotrf('L', 3, a, 3, info)
  call zpotrs('L', 3, 2, a, 3, b, 3, info)
  call begin_test('lower_multi_rhs')
  call print_array('x', b_r, 12)
  call print_int('info', info)
  call end_test()

  ! Test 4: N=0 quick return
  call zpotrs('L', 0, 1, a, 1, b, 1, info)
  call begin_test('n_zero')
  call print_int('info', info)
  call end_test()

  ! Test 5: NRHS=0 quick return
  call zpotrs('L', 3, 0, a, 3, b, 3, info)
  call begin_test('nrhs_zero')
  call print_int('info', info)
  call end_test()

  ! Test 6: 1x1 system (L = sqrt(4) = 2, so A=(4,0), L=(2,0))
  a(1) = (2.0d0, 0.0d0)
  b(1) = (6.0d0, 3.0d0)
  call zpotrs('L', 1, 1, a, 1, b, 1, info)
  call begin_test('one_by_one')
  call print_array('x', b_r, 2)
  call print_int('info', info)
  call end_test()

  ! Test 7: upper, multiple RHS (NRHS=3), computes A^{-1}
  a = (0.0d0, 0.0d0); b = (0.0d0, 0.0d0)
  a(1) = (10.0d0, 0.0d0); a(2) = (3.0d0, 1.0d0); a(3) = (1.0d0, -2.0d0)
  a(4) = (3.0d0, -1.0d0); a(5) = (8.0d0, 0.0d0); a(6) = (2.0d0, 1.0d0)
  a(7) = (1.0d0, 2.0d0); a(8) = (2.0d0, -1.0d0); a(9) = (6.0d0, 0.0d0)
  ! B = I (3x3 identity, col-major complex)
  b(1) = (1.0d0, 0.0d0); b(2) = (0.0d0, 0.0d0); b(3) = (0.0d0, 0.0d0)
  b(4) = (0.0d0, 0.0d0); b(5) = (1.0d0, 0.0d0); b(6) = (0.0d0, 0.0d0)
  b(7) = (0.0d0, 0.0d0); b(8) = (0.0d0, 0.0d0); b(9) = (1.0d0, 0.0d0)
  call zpotrf('U', 3, a, 3, info)
  call zpotrs('U', 3, 3, a, 3, b, 3, info)
  call begin_test('upper_multi_rhs_3')
  call print_array('x', b_r, 18)
  call print_int('info', info)
  call end_test()

end program
